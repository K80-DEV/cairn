...
DAEMON_OWNER = os.environ.get("MARA_OWNER", "K80")
...
BUILD_NAME = "Ghostlight"  # held for the whole Basalt run (K80 12:34)
...
def init_db():
    with sqlite3.connect(DB_PATH) as db:
        ...
        if "compacted_at" not in cols:
            db.execute("ALTER TABLE messages ADD COLUMN compacted_at REAL")
        ...
        if "attachments" not in cols:
            db.execute("ALTER TABLE messages ADD COLUMN attachments TEXT")
        ...
        if "stopped" not in cols:
            db.execute("ALTER TABLE messages ADD COLUMN stopped INTEGER DEFAULT 0")
        ...
        if "model_override" not in ccols:
            db.execute("ALTER TABLE conversations ADD COLUMN model_override TEXT")
...
def init_registry():
    ...
    with _reg_db() as db:
        ...
        if "expires_at" not in _scols:
            db.execute("ALTER TABLE sessions ADD COLUMN expires_at REAL")
...
def registry_create_user(username, password, display_name, agent_name, slug, role="user", status="pending", approved_at=None):
    uid = str(uuid.uuid4())
    salt = os.urandom(16).hex()
    with _reg_db() as db:
        db.execute(
            "INSERT INTO users (id, username, display_name, agent_name, slug, role, status, salt, pw_hash, created_at, approved_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (uid, username, display_name, agent_name, slug, role, status, salt, _hash_pw(password, salt), time.time(), approved_at))
    return uid
...
def run_compaction(conv_id: str, messages: list, model_cfg: dict, status=None, username=None) -> str:
    """
...
def _nc_host_allowed(host):
    ...
    if h == NC_ALLOW_SUFFIX.lstrip(".") or h.endswith(NC_ALLOW_SUFFIX):
        return True
    try:
    ...
    try:
        with sqlite3.connect(DB_PATH) as db:
            ...
            history = db.execute(
                ...
                "WHERE conv_id=? AND role IN ('user','assistant') ORDER BY ts",
                (conv_id,)).fetchall()
...
class _P1cRedirectGuard(urllib.request.HTTPRedirectHandler):
    ...
    def __init__(self, check=None, same_origin=False):
        ...
        # 192.168.1.50" is legitimate config; "HA just told you to go read
        # 192.168.1.1" is lateral movement. When same_origin is set, every
...
loadTasks();
loadSettings(); loadMemory(); loadSession(); loadApprovals(); loadSysPrompt(); loadUserRoles(); loadToolsCard(); loadCompactionCard(); loadAvatarCard(); loadVault();
</script>
</body>
</html>
"""
    ...
    def do_GET(self):
        ...
        if path == "" or path == "/":
            ...
            if not u or u["role"] != "owner" or u["status"] != "active":
                ...
                return
            with _reg_db() as db:
                rows = db.execute("SELECT id, username, display_name, agent_name, slug, created_at FROM users WHERE status='pending' ORDER BY created_at").fetchall()
            ...
            if not _instance_access(u, _instance_principal()):
                ...
                return
            text = SYSTEM_PROMPT_PATH.read_text() if SYSTEM_PROMPT_PATH.exists() else ""
            pristine_chars = None
            if SYSTEM_PROMPT_PRISTINE.exists():
                pristine_chars = len(SYSTEM_PROMPT_PRISTINE.read_text())
            self._json(200, {
                "text": text,
                "chars": len(text),
                ...
                "context_budget": ctx_budget(u["username"]),
                "pristine": {"exists": SYSTEM_PROMPT_PRISTINE.exists(), "chars": pristine_chars},
...
# ---- S4f11: creds vault. Entered, encrypted, never returned. ----
    ...
    def do_POST(self):
        ...
        if path == "/api/chat":
            ...
            self._json(200, {"ok": True, "status": "pending", "message": "account created - awaiting admin approval"})
            ...
            if action == "save":
                ...
                try:
                    ...
                    return
                self._json(200, {"ok": True, "chars": len(text), "history": _sp_history_list()})
            elif action == "reset":
                version = body.get("version") or "pristine"
                text = None
                if version == "pristine":
                    if SYSTEM_PROMPT_PRISTINE.exists():
                        text = SYSTEM_PROMPT_PRISTINE.read_text()
                else:
                ...
                try:
                    ...
                    return
                self._json(200, {"ok": True, "chars": len(text), "restored": version, "history": _sp_history_list()})
            else:
    ...
    def _handle_chat(self):
        ...
        with sqlite3.connect(DB_PATH) as db:
            ...
            db.row_factory = sqlite3.Row
            history = db.execute(
                "SELECT id, role, content, attachments, stopped FROM messages WHERE conv_id=? AND role IN ('user','assistant') ORDER BY ts",
                (conv_id,)
            ).fetchall()
            db.execute("UPDATE conversations SET updated_at=? WHERE id=?", (now, conv_id))
            db.commit()
...
def _f20_enabled():
    return get_setting("update_enabled", "0", DAEMON_OWNER) == "1"


def _f20_manifest_url():
    return str(get_setting("update_url", "", DAEMON_OWNER) or "").strip()


def _f20_lan_ok():
    return get_setting("update_allow_lan", "0", DAEMON_OWNER) == "1"


def _f20_xorigin_ok():
    return get_setting("update_allow_xorigin", "0", DAEMON_OWNER) == "1"
...
def _f20_stage(force=False):
    ...
    log_event(DAEMON_OWNER, "update.staged", version=summ["version"], kind=summ["kind"],
...
def _f20_prepare(man, summ, force=False):
    ...
    if err:
        ...
        log_event(DAEMON_OWNER, "update.failed", stage="smoke", version=summ["version"], err=err[:120])
    ...
    log_event(DAEMON_OWNER, "update.installed", **{"from": VERSION, "to": summ["version"],
...
def _f20_apply_rotation(man, summ):
    ...
    log_event(DAEMON_OWNER, "update.rotated", nonce=str(man["key_rotation"]["nonce"]),
...
def _f20_route_post(h, path):
        ...
        for key, skey in (("enabled", "update_enabled"), ("lan", "update_allow_lan"),
            ...
            if key in d:
                if not isinstance(d[key], bool):
                    ...
                    return
                set_setting(skey, "1" if d[key] else "0", DAEMON_OWNER)
                fields.append(key)
        if "url" in d:
            ...
            if url:
                ...
                if err:
                    log_event(DAEMON_OWNER, "update.rejected", reason="config url: " + err[:100])
                    h._json(400, {"error": err})
                    return
            set_setting("update_url", url, DAEMON_OWNER)
            fields.append("url")
        log_event(DAEMON_OWNER, "update.config", fields=",".join(fields)[:120])
        h._json(200, {"ok": True, "fields": fields})
        return
    if path in ("/api/update/check", "/api/update/download", "/api/update/install") and not _f20_enabled():
        log_event(DAEMON_OWNER, "update.rejected", reason="disabled " + path[-8:])
        h._json(409, {"error": "updater is disabled (enable it on this card first)"})
        return
    if path == "/api/update/check":
        err, summ = _f20_check()
        if err:
            log_event(DAEMON_OWNER, "update.rejected", reason="check: " + err[:100])
            h._json(400, {"error": err})
            return
        log_event(DAEMON_OWNER, "update.check", version=summ["version"], kind=summ["kind"],
    ...
    if path == "/api/update/install":
        ...
        err, pair = _f20_staged()
        if err:
            log_event(DAEMON_OWNER, "update.rejected", reason="install: " + err[:100])
...
def _f20_boot_note():
    pend = _f20_jread(_F20_PENDING)
    if isinstance(pend, dict):
        log_event(DAEMON_OWNER, "update.booted",
