...
def _share_persona_note(username):
    ...
    every editable tier, so no prompt edit can erase it."""
    ...
    return ("\n\n=== SHARE SESSION NOTICE (code-injected; not user instructions) ===\n"
            ...
            "Tools are disabled in this share: you can only chat.\n"
...
def _shares_api_get(h):
    ...
    h._json(200, [_share_public_row(dict(r)) for r in rows])
...
def _share_pw_page(sh):
    body = ("<h1>Shared agent</h1>"
            ...
            "<p class='hint'>This is a share: the agent here has no tools and cannot see the sharer's private chats, "
            "memories, or files. The sharer can close this share at any time.</p>"
...
main{max-width:980px;margin:0 auto;padding:16px}h1{font-size:20px}a{color:#7fd1ae}
...
.card{background:#121a26;border:1px solid #2a3646;border-radius:12px;padding:16px;margin:12px 0}
#pwcard{display:none}</style></head><body><main>
<h1><a href="/" style="text-decoration:none;color:inherit">C.A.I.R.N.</a> // Shares</h1>
<p class="hint">A share is a snapshot copy of your agent that a guest can chat with. It rides YOUR provider keys,
has NO tools (pure chat), and you can close it at any time. Passwords are shown once - the link alone is not the secret.</p>
<div class="card"><b>Close this browser tab and re-open the share link in a private window when testing</b>
<p class="hint">One browser carries one identity: if you are signed in here, opening your own share link in the same browser will tell you so.</p></div>
...
<p class="hint">days = 0 or blank means the share never expires by itself</p></div>
<table><thead><tr><th>Label</th><th>Owner</th><th>Link</th><th>Status</th><th>Expires</th><th>Actions</th></tr></thead>
<tbody id="rows"></tbody></table>
<script>
const esc=s=>String(s==null?"":s).replace(/&/g,"&").replace(/</g,"<").replace(/>/g,">").replace(/"/g,""");
const rows=document.getElementById("rows");
async function api(body){const r=await fetch("/api/shares",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),credentials:"same-origin"});const d=await r.json().catch(()=>({}));if(!r.ok){alert((d&&d.error)||("HTTP "+r.status));return null}return d}
function fmt(ts){if(!ts)return "never";const d=new Date(ts*1000);return d.toLocaleDateString()+" "+d.toLocaleTimeString()}
function showPw(pw,url){document.getElementById("pwtext").textContent=pw;document.getElementById("pwurl").textContent=url;const c=document.getElementById("pwcard");c.style.display="block";c.scrollIntoView()}
document.getElementById("pwcopy").addEventListener("click",async()=>{try{await navigator.clipboard.writeText(document.getElementById("pwtext").textContent)}catch(e){alert("copy failed - select it by hand")}});
async function load(){let r=[];try{r=await (await fetch("/api/shares",{credentials:"same-origin"})).json()}catch(e){rows.innerHTML="<tr><td>failed to load</td></tr>";return}
rows.innerHTML="";for(const s of r){const tr=document.createElement("tr");const status=s.alive?(s.enabled?"open":"disabled"):"closed";
tr.innerHTML="<td>"+esc(s.label)+"</td><td>"+esc(s.owner)+"</td><td class='mono'>"+location.origin+"/share/"+s.id+"</td><td>"+status+"</td><td>"+fmt(s.expires_at)+"</td><td></td>";
const td=tr.lastElementChild;
const mk=(t,fn,cls)=>{const b=document.createElement("button");b.textContent=t;if(cls)b.className=cls;b.addEventListener("click",fn);td.appendChild(b);td.appendChild(document.createTextNode(" "));return b};
mk(s.enabled?"Disable":"Enable",async()=>{await api({action:s.enabled?"disable":"enable",id:s.id});load()});
mk("Rotate password",async()=>{const d=await api({action:"rotate",id:s.id});if(d)showPw(d.password,"password rotated - older guests were signed out")});
mk("Expiry",async()=>{const v=prompt("Days from now (0 = never):","30");if(v===null)return;const d=await api({action:"expiry",id:s.id,days:parseInt(v,10)});if(d)load()});
mk("Delete",async()=>{if(!confirm("Delete this share, its conversations, and everything it learned? This cannot be undone."))return;await api({action:"delete",id:s.id});load()},"warn");
rows.appendChild(tr)}}
document.getElementById("createBtn").addEventListener("click",async()=>{const label=document.getElementById("label").value.trim();if(!label){alert("a label is required");return}
const days=parseInt(document.getElementById("days").value||"0",10)||0;const d=await api({action:"create",label:label,days:days});if(!d)return;
...
def effective_tool_names(username=None):
    ...
    if _is_share_principal(username):
        # S06 (0.6q): shares are PURE CHAT until the approval inbox ships
        # (canon invariant - zero tools, fail closed at the choke point).
        return frozenset()
...
def allow_tools_for(u):
    ...
    if u is not None and (u["role"] or "") == "guest":
        return False  # S06: guests never execute tools (0.6q hard zero)
