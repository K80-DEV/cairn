...
def _share_public_row(sh):
    return {"id": sh["id"], "label": sh["label"], "owner": sh["owner"],
            ...
            "url": "/share/" + sh["id"]}
def share_create(owner_username, label, expires_days):
    ...
    log_event(owner_username, "share.create", share_id=sid)
    return _share_public_row(share_get(sid)), None, pw
...
<p class="hint">days = 0 or blank means the share never expires by itself. All grant boxes start OFF - open them per share below.</p></div>
...
<div id="grantscard" class="card"><b>Grants: <span id="glabel"></span> <span class="hint" id="gtoolsstate"></span></b>
...
function load(){fetch("/api/shares",{credentials:"same-origin"}).then(function(r){return r.json().catch(function(){return []})}).then(function(r){
...
document.getElementById("createBtn").addEventListener("click",function(){var label=document.getElementById("label").value.trim();
 ...
 post("/api/shares",{action:"create",label:label,days:days}).then(function(d){if(!d)return;
