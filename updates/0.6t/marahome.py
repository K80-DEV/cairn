...
def _sharer_tool_ceiling(owner_username):
    ...
    try:
        ...
        if not allow_tools_for(u):
            return frozenset()
        return frozenset(effective_tool_names(owner_username))
    except Exception:
        return frozenset()
...
def _share_mem_read(sh, name):
    ...
    try:
        if not p.exists() or not p.resolve().is_relative_to(d.resolve()):
            return None
        return p.read_text(errors="replace")[:SHARE_MEM_PREVIEW_CAP]
    except Exception:
        return None
def _share_mem_merge_back(sh, name, decided_by):
    ...
    content = _share_mem_read(sh, name)
    if content is None:
        return None, "file not found in the share's memory (or too large to review)"
...
def _share_memories_api_get(h):
    ...
    staff = u["role"] in ("owner", "admin")
    if sh is None or (sh["owner"] != u["username"] and not staff):
...
def _share_memories_api_post(h):
    ...
    if action == "read":
        c = _share_mem_read(sh, name)
        if c is None:
            h._json(404, {"error": "not found"})
            return
        h._json(200, {"name": name, "content": c})
        return
    if action == "merge":
...
def _share_approvals_api_post(h):
    ...
    if row is None or (row["owner_"] != u["username"] and not staff):
        h._json(404, {"error": "not found"})
        return
    row = dict(row)
...
def _shares_api_get(h):
    ...
    with _reg_db() as db:
        if staff:
            rows = db.execute("SELECT * FROM shares ORDER BY created_at DESC").fetchall()
        else:
...
def _shares_api_post(h):
    """Share management API. Every active principal may manage their OWN
    shares (K80 ruling 9: ceiling = sharer); owner/admin manage all. Guests
    never reach here - the allowlist fences /api/shares before auth even
    resolves them."""
    u = h._need_user()
    if not u:
        return
...
class MaraHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/api/shares":  # S06: my shares (staff see all)
            _shares_api_get(self)
        elif path == "/api/share-approvals":  # S06r approval inbox (staff side)
            _share_approvals_api_get(self)
        elif path == "/api/share-memories":  # S06s clone-memory review + merge-back (staff side)
