...
def main():
    ...
    # F29 R4: TLS mode wiring. Never fatal: a missing or unloadable cert starts
    # the daemon PLAIN with a loud log line (recoverable-plain beats bricked).
    _r4_cert, _r4_key = _tls_active_pair()
    _TLS_RUNTIME = _tls_mode()
    if _TLS_RUNTIME != "proxy":
        if _r4_cert:
            try:
                server.socket = _tls_ctx_from(_r4_cert, _r4_key).wrap_socket(server.socket, server_side=True)
                TLS_SERVED = True
                _TLS_SERVED_CERT = _r4_cert
                log.info("TLS: mode=%s serving HTTPS on %s:%d (cert %s)", _TLS_RUNTIME, HOST, PORT, _r4_cert)
            except Exception as _r4_e:
                log.error("TLS: mode=%s but %s could not be loaded (%s: %s) - starting PLAIN HTTP so you can still reach Settings; fix the cert or switch mode", _TLS_RUNTIME, _r4_cert, type(_r4_e).__name__, _r4_e)
        else:
            log.error("TLS: mode=%s but no usable cert+key pair exists - starting PLAIN HTTP so you can still reach Settings; generate the local CA or save an existing pair", _TLS_RUNTIME)
