...
def _share_login(h):
    ...
    tok = session_create(sh["principal"], u["slug"], u["id"])
    ...
    def do_GET(self):
        ...
        if path == "" or path == "/":
            ...
            with sqlite3.connect(DB_PATH) as db:
                ...
                # S06 (0.6s): the history box. A share principal with
                # allow_history=0 (the default) sees ONLY its most recent
                # conversation - older threads stay in the DB (retained for the
                # 0.6s merge-back review canon), just not listed. Ordinary
                # principals are untouched, byte-for-byte.
                # SHAPE SCAR: this MUST be a single-line conditional expression.
                # An if-block at 12 spaces swallowed the 16-space response line
                # that follows it in the source, so ungated requests hung.
                rows = rows[:1] if (_is_share_principal(u["username"]) and not _share_history_allowed(u["username"])) else rows
