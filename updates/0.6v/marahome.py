...
# "Ghostlight" as published history - forward-only). Builds differ by version + sha only.
...
# F29/R2 (Q3 ruling, K80 2026-09-24 09:54): the owner is NOT bound by the
# global Tier 0. A personal tier0__<owner>.md, when it exists, projects for
# the owner INSTEAD of the global file; absent, the global binds the owner
# too (sane default - community installs never have to think about this).
TIER0_PERSONAL_FMT = "tier0__%s.md"
def _tier0_target(username=None):
    """F29/R2: which Tier-0 file projects for this principal?
    Owner with a personal file -> that file; everyone else (and the owner
    without one) -> the global. Never created here (existence-checked read
    path, same contract as _tier1_file)."""
    who = username or DAEMON_OWNER
    if who == DAEMON_OWNER:
        p = IDENTITY / (TIER0_PERSONAL_FMT % DAEMON_OWNER)
        if p.exists():
            return p
    return TIER0_PATH
def _tier0_write_target(username=None, scope="global"):
    """F29/R2: write-path resolver for the owner UI. scope='personal' ->
    the owner's own file (validated name, same sanitizer house style);
    scope='global' -> the instance constitution. Owner-only routes gate
    before this is ever called."""
    if scope == "personal":
        who = username or DAEMON_OWNER
        if not isinstance(who, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9-]{2,31}", who):
            return None
        return IDENTITY / (TIER0_PERSONAL_FMT % who)
    return TIER0_PATH
...
def load_system_prompt(username=None) -> str:
    ...
    """
    parts = []
    _t0p = _tier0_target(username)  # F29/R2: owner personal override, else global
    _t0lab = ("TIER 0 - PERSONAL CONSTITUTION (yours; code-injected; binds ONLY your sessions; not editable by any prompt layer)"
              if _t0p != TIER0_PATH else
              "TIER 0 - DAEMON CONSTITUTION (code-injected; not editable by any prompt layer)")
    _tb = _tier_block(_t0p, _t0lab)
    if _tb:
        parts.append(_tb)
    ...
    # S06 (0.6s): the memory box is a real fence now. A share principal with
    # allow_memory=0 (the default) gets its memory namespace NEVER projected -
    # not even files the include-memories box copied at creation. Fail-closed.
    if (username or "").startswith("share-") and not _share_memory_allowed(username):
        d = None  # cheap prefix belt: _share_memory_allowed is the real fence
...
def _tier_commit(path, text, note, who, pristine_src=None):
    ...
    try:
        if path == TIER0_PATH or path.name.startswith("tier0__"):  # F29/R2: personal tier0 saves log too
...
def init_db():
    with sqlite3.connect(DB_PATH) as db:
        ...
        # C03 (0.6v): per-visit guest identity stamp for share conversations.
        # NULL = legacy (pre-C03) or non-guest thread. Consent model lives in
        # Handler._guest_can_conv; here we only grow the column. (K80 2026-09-25)
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // L31: addEventListener returns undefined - the old trailing .catch()
    // dangled on that void value and threw a TypeError at page load, so the
    // network-failure notice could never appear. Failures now surface here.
    show("request failed - check connection", "err");
  }
});
</script>
...
# ─── S06 Share Clones (0.6q) ────────────────────────────────────────────────
# Design canon: MaraDen/share-clones-design-20260924.md (sha f55a9ab9e6922a).
# K80 rulings 2026-09-23: the sharer pays for API + model + tiers ("Period.");
# all boxes OFF by default (pure chat); link + generated password; enable/
# disable switch; separate namespaces. Grants/inbox/presence dead-man land in
# 0.6r; memories/history/merge-back in 0.6s. A share is a synthetic registry
# principal (role=guest) that can NEVER exceed its creator, rides the
# creator's provider keys (no key duplication), and dies clean on delete.
SHARE_MAX_PER_OWNER = 20
_SHARE_LABEL_MAX = 64
SHARE_CHAT_BURST = 20      # messages a guest can fire in a burst...
SHARE_CHAT_PER_MIN = 6     # ...then this sustained rate per share
_SHARE_SID_RE = re.compile(r"^[0-9a-f]{8}$")
_SHARE_PRIN_RE = re.compile(r"^share-([0-9a-f]{8})$")
def _is_share_principal(username):
    return bool(username) and bool(_SHARE_PRIN_RE.match(username or ""))
def share_get(sid):
    with _reg_db() as db:
        return db.execute("SELECT * FROM shares WHERE id=?", (sid,)).fetchone()
def share_get_by_principal(principal):
    with _reg_db() as db:
        return db.execute("SELECT * FROM shares WHERE principal=?", (principal,)).fetchone()
def _share_alive(sh):
...
def share_create(owner_username, label, expires_days, include_mem=False):
    ...
    Tier-1 copy is snapshotted ONLY when they keep one (the owner's Tier 1 IS
    the global base - owner shares project it live; documented asymmetry)."""
    label = re.sub(r"[\x00-\x1f\x7f]", "", str(label or "")).strip()[:_SHARE_LABEL_MAX]
    if not label:
        return None, "label required (max " + str(_SHARE_LABEL_MAX) + " chars)", None
    try:
    ...
    for _try in range(8):
        ...
        if share_get(cand) or registry_get_user("share-" + cand):
            continue
        sid = cand
        break
    if not sid:
    ...
    try:
        ...
        # deliberate exception to an empty share namespace. One-way copy,
        # capped; originals untouched; projection still gated by allow_memory.
        _mem_n = 0
        if include_mem:
    ...
    _SP_CACHE.pop(principal, None)
    log_event(owner_username, "share.create", share_id=sid, memories_copied=_mem_n)
    _row = _share_public_row(share_get(sid))
    if _mem_n:
        _row["memories_copied"] = _mem_n
    return _row, None, pw
def share_rotate_password(sh):
    pw = os.urandom(12).hex()
    salt = os.urandom(16).hex()
    try:
        with _reg_db() as db:
            db.execute("UPDATE shares SET salt=?, pw_hash=? WHERE id=?", (salt, _hash_pw(pw, salt), sh["id"]))
        # the OLD password's sessions die with it - rotation is revocation
        _su = registry_get_user(sh["principal"])
        if _su:
            sessions_delete_all(sh["principal"])
    except Exception:
        return None, "rotation failed"
    log_event(sh["owner"], "share.rotate", share_id=sh["id"])
    return pw, None
def share_set_enabled(sh, enabled):
    with _reg_db() as db:
        db.execute("UPDATE shares SET enabled=? WHERE id=?", (1 if enabled else 0, sh["id"]))
    if not enabled:
        sessions_delete_all(sh["principal"])  # disable = instant kick
    log_event(sh["owner"], "share.enabled" if enabled else "share.disabled", share_id=sh["id"])
def share_set_expiry(sh, days):
    try:
        days = int(days or 0)
    except Exception:
    ...
    expires_at = (time.time() + days * 86400.0) if days else None
    with _reg_db() as db:
        db.execute("UPDATE shares SET expires_at=? WHERE id=?", (expires_at, sh["id"]))
...
def share_delete(sh):
    ...
    try:
        ...
        if md is not None and md.parent == MEMORY_DIR and md.exists():
...
def _share_path_allowed(p):
    ...
    APIs 401, pages redirect to login. The guest's own conversations are
    scoped by user_id on every route below, unchanged."""
    if p == "/share" or p.startswith("/share/"):
        return True
    if p in ("/api/me", "/api/logout", "/api/chat", "/api/stop", "/api/conversations", "/health"):
        return True
    if p.startswith("/api/conversations/"):
        return p.endswith("/messages") or p.endswith("/delete")
    if p.startswith("/api/stream/") or p.startswith("/api/attachments/"):
        return True  # both owner-checked downstream
    return False
def _share_gate(h, u):
    """Request-time share fence (revocation is real): expired or disabled
    shares die HERE, mid-life, and their sessions are swept. Guests also get
    the message nozzle. Returns True when the caller must stop."""
    sh = share_get_by_principal(u["username"])
    if sh is None:
        h._json(403, {"error": "this share no longer exists"})
        return True
    if not _share_alive(sh):
        sessions_delete_all(u["username"])
        h._json(403, {"error": "this share is closed"})
        return True
    if not _rate_allow("sharechat|" + u["username"], SHARE_CHAT_BURST, SHARE_CHAT_PER_MIN):
        h._json(429, {"error": "this share has a message limit - try again in a moment"})
        return True
    return False
# ── S06 0.6r: grants, approval inbox, presence dead-man ─────────────────────
# Canon: MaraDen/share-clones-design-20260924.md (0.6r addendum). K80 rulings
# verbatim: every box OFF by default; 1-minute presence dead-man; approvals
# land in the inbox, never inline in the sharer's own chat.
SHARE_PRESENCE_TTL = 60      # K80's "1 minute", dead-man strict
SHARE_APPROVAL_TTL = 86400   # stale pending approvals expire after 24h
SHARE_ASK_MAX_PENDING = 5    # per share - no inbox spam
SHARE_ASK_NAME = "ask_sharer_for_tool"
SHARE_MEMORY_TOOLS = frozenset(("memory", "recall"))
SHARE_ASK_SCHEMA = {"type": "function", "function": {
    "name": SHARE_ASK_NAME,
    "description": ("Ask the person who owns this share clone (the sharer) to "
        "approve one tool for this clone. Creates a request in their approval "
        "inbox - it grants NOTHING by itself and must never be promised to the "
        "guest. Use only when a tool would genuinely help the guest."),
    "parameters": {"type": "object", "properties": {
        "tool_name": {"type": "string", "description": "the exact tool name you would like"},
        "why": {"type": "string", "description": "one short sentence for the sharer"}},
        "required": ["tool_name"]}}}
def _share_grants(sh):
    """Parsed grant columns (all default OFF, K80 ruling 2). Accepts Row or dict."""
    sh = dict(sh)
    try:
        raw = json.loads(sh.get("tools") or "[]")
        names = sorted({n for n in raw if isinstance(n, str)}) if isinstance(raw, list) else []
    except Exception:
        names = []
    return {"allow_tools": int(sh.get("allow_tools") or 0),
            "tools": names,
            "allow_memory": int(sh.get("allow_memory") or 0),
            "allow_history": int(sh.get("allow_history") or 0)}
# C04 (K80 law 2026-09-25, audit round 8): the SHARE-SAFE TOOL FLOOR. A share
# clone can only ever carry chat-adjacent tools - the user-tier universe
# (observed: web_search, web_fetch, memory, recall). Host-global capability
# (execute_shell, ssh_run, read/write/edit_file, grep/glob, send_file,
# run_with_secret, every credential connector) is refused BY ABSENCE from the
# floor, no matter who owns the share - not even the owner may mint a clone
# with shell hands. Lazy helper (not a module constant) because
# _sharer_tool_ceiling sits above the TIER_TOOLS definition site in this
# file; name resolution happens at call time.
def _share_safe_set():
    try:
        fs = frozenset(TIER_TOOLS.get("user"))
    except Exception:
        fs = frozenset()
    return fs & frozenset({"web_search", "web_fetch", "memory", "recall"}) \
        if fs else frozenset({"web_search", "web_fetch", "memory", "recall"})


def _sharer_tool_ceiling(owner_username):
    """What the CREATOR can do right now (live). A clone is always share.tools
    intersected with THIS - disabling a tool on yourself shrinks every clone.
    0.6t X (audit round 7, K80-approved): the clone ceiling ALSO passes the
    SEC4 execution fence (allow_tools_for, P1-A/C1). The live tool list alone
    let a brain-only creator (e.g. an admin on an owner-tier box) mint a clone
    with tools they cannot personally use through any other door. If the
    creator cannot run tools at all, the clone ceiling is EMPTY by law."""
    try:
        u = registry_get_user(owner_username)
        if not allow_tools_for(u):
            return frozenset()
        # C04: intersect with the share-safe floor AFTER the live-creator
        # ceiling. Even an owner-tier creator whose own hands include the
        # shell can only offer a clone the chat-adjacent set.
        return frozenset(effective_tool_names(owner_username)) & _share_safe_set()
    except Exception:
        return frozenset()
def _cancel_runs_for(principals):
    """C02 immediate halt (K80 hard requirement): fire the cancel event of
    every in-flight run owned by any principal in the list. Reuses the
    /api/stop registry - same threading.Event, same agent-loop check.
    Best-effort per run: the session kill and the request fence have already
    fired before this is called, so a lookup failure can never leave a NEW
    run alive - it can only fail to hurry an old one."""
    principals = [p for p in (principals or []) if p]
    if not principals:
        return 0
    hits = 0
    try:
        with sqlite3.connect(DB_PATH) as db:
            ...
            rows = db.execute("SELECT id FROM conversations WHERE user_id IN (" + q + ")",
                              principals).fetchall()
        for r in rows:
            ev = _CANCEL_EVENTS.get(r[0])
            if ev is not None:
                ev.set()
                hits += 1
    except Exception:
...
def _share_presence_touch(owner_username):
    try:
        with _reg_db() as db:
            db.execute("INSERT INTO share_presence (owner, seen_at) VALUES (?,?)"
                       " ON CONFLICT(owner) DO UPDATE SET seen_at=excluded.seen_at",
                       (owner_username, time.time()))
            db.commit()
    except Exception:
        log.warning("S06r: presence touch failed for %s", owner_username)
def _share_presence_fresh(owner_username):
    try:
        with _reg_db() as db:
            row = db.execute("SELECT seen_at FROM share_presence WHERE owner=?",
                             (owner_username,)).fetchone()
        return bool(row and (time.time() - float(row["seen_at"] or 0)) <= SHARE_PRESENCE_TTL)
    except Exception:
        return False   # dead-man: unreadable presence means the tools are OFF
def _share_memory_allowed(principal):
    sh = share_get_by_principal(principal)
    return bool(sh) and bool(_share_grants(sh)["allow_memory"])
def _share_effective_tools(principal):
    """0.6r: the whole share-tool model in ONE function, behind the same
    choke point as everything else. Grants intersected with the LIVE ceiling,
    gated by the presence dead-man (K80 ruling 7: away >60s = chat-only, no
    tools at all - not even the tool that asks for tools)."""
    sh = share_get_by_principal(principal)
    if sh is None or not _share_alive(sh):
        return frozenset()
    if not _share_presence_fresh(sh["owner"]):
        return frozenset()
    ceiling = _sharer_tool_ceiling(sh["owner"])
    if not ceiling:
        return frozenset()
    names = {SHARE_ASK_NAME}
    g = _share_grants(sh)
    if g["allow_tools"]:
        names |= (set(g["tools"]) & set(ceiling))
    if not g["allow_memory"]:
        names -= SHARE_MEMORY_TOOLS
    return frozenset(names)
# ── S06 0.6s: memory copy-at-creation, projection gate, history box, merge-back ─
# Canon rulings 5 + 8: memories created inside a share stay SEPARATE; merge-back
# is per-file, human-reviewed, provenance-marked ("learned during a share, not
# lived experience"). The copy-at-creation box is the one deliberate exception
# to an empty namespace (K80 checked the box; ruling 5, "document the cliff,
# don't nanny"). Enforcement stays at existing choke points, not new ones.
SHARE_MEM_PREVIEW_CAP = 8000       # review pane caps at ~8KB per file
SHARE_MEM_COPY_CAP  = 262144       # total bytes copied at creation (per share)
SHARE_MEM_PROV = ("Merged back from share '{label}' (share {sid}) on {date}. "
                  "Learned during a share, not lived experience.\n\n")
def _share_mem_dir(principal):
    """Containment: the share's OWN memory directory or None. Re-derives from
    the validated principal string - never from a caller-supplied path."""
    d = _user_memory_dir(principal)
    if d is None or not _is_share_principal(principal):
        return None
    return d
def _share_copy_memories_at_creation(sh, owner_username):
    """The include-memories box: copy the sharer's .md memory files INTO the
    share's namespace (one-way, capped, .md only, silent on any error is NOT
    allowed - returns a count and a note). Never writes outside the share dir."""
    src = _user_memory_dir(owner_username)
    dst = _share_mem_dir(sh["principal"])
    ...
    try:
        for f in sorted(src.glob("*.md")):
            try:
                ...
                continue
            if total + len(data) > SHARE_MEM_COPY_CAP:
                continue  # per-file skip: one oversized file must not veto the rest
        ...
        log.warning("S06s: copy-at-creation failed for share %s", sh["id"])
    return copied
def _share_mem_list(sh):
    ...
    for f in sorted(d.glob("*.md")):
        try:
            stt = f.stat()
            out.append({"name": f.name, "size": stt.st_size, "mtime": stt.st_mtime})
        except Exception:
            continue
    return out
...
def _share_mem_merge_back(sh, name, decided_by):
    ...
    if _trunc:
        # M21: a truncated preview must never become a "successful" merge.
    ...
    base = "share-" + sh["id"] + "-" + name
    dest = dst_dir / base
    n = 2
    while dest.exists() and n < 100:
        dest = dst_dir / ("share-%s-%s-%d.md" % (sh["id"], name[:-3], n)); n += 1
    if dest.exists():
        return None, "too many numbered collisions - clean up memory and retry"
    try:
        _lab = sh["label"]
    except Exception:
        _lab = "untitled"
    prov = SHARE_MEM_PROV.format(label=str(_lab or "untitled"),
                                 sid=sh["id"], date=time.strftime("%Y-%m-%d"))
    try:
    ...
    reload_identity()
    log_event(sh["owner"], "share.memory.merged", share_id=sh["id"], file=dest.name)
    return dest.name, None
def _share_memories_api_get(h):
    u = h._need_user()
    if not u:
        return
    qs = urllib.parse.parse_qs(urllib.parse.urlparse(h.path).query)
    sid = (qs.get("id") or [""])[0]
    sh = share_get(sid) if _SHARE_SID_RE.match(sid) else None
    if sh is None or sh["owner"] != u["username"]:  # C02 self-only
        h._json(404, {"error": "not found"})
        return
    h._json(200, {"files": _share_mem_list(sh), "label": sh["label"]})
def _share_memories_api_post(h):
    u = h._need_user()
    if not u:
        return
    body = h._read_body()
    ...
    if action == "merge":
        ...
        if err:
            ...
            return
        h._json(200, {"ok": True, "merged_as": dest})
        return
    h._json(400, {"error": "unknown action"})
def _share_history_allowed(principal):
    """S06 (0.6s): the guest-side history box (0.6r addendum, enforceable
    now). Fail-closed: anything unparseable means 'not allowed'.
    NOTE (scar): rows from the registry are sqlite3.Row - NO .get() method.
    A .get() inside a fail-closed try/except is a silent always-deny bug."""
    try:
        sh = share_get_by_principal(principal)
        if sh is None:
            return False
        return bool(int(sh["allow_history"] or 0))
    except Exception:
        return False
def _share_approvals_expire():
    try:
        with _reg_db() as db:
            db.execute("UPDATE share_approvals SET status='expired', decided_at=?"
                       " WHERE status='pending' AND created_at < ?",
                       (time.time(), time.time() - SHARE_APPROVAL_TTL))
            db.commit()
    except Exception:
        log.warning("S06r: approval expiry sweep failed")
def _share_ask_execute(username, args):
    """The ask pseudo-tool: a LETTER TO THE INBOX, not a grant."""
    sh = share_get_by_principal(username)
    if sh is None or not _share_alive(sh):
        return "this share is closed"
    if not _rate_allow("shareask|" + sh["id"], 3, 2):
        return "approval requests are rate limited - try again in a bit"
    tool = str((args or {}).get("tool_name") or "").strip()
    why = str((args or {}).get("why") or "")[:280]
    ceiling = _sharer_tool_ceiling(sh["owner"])
    if not ceiling:
        return "the sharer has no tools to share right now"
    ...
    with _reg_db() as db:
        ...
        db.execute("INSERT INTO share_approvals (share_id, tool_name, note, status,"
                   ...
                   (sh["id"], tool, why, "pending", username, time.time()))
        db.commit()
    log_event(sh["owner"], "share.approval.requested", share_id=sh["id"], tool=tool)
    return ("request sent to the sharer's approval inbox (" + tool + "). It is NOT active -"
            " nothing changes unless the sharer approves. Do not promise the guest anything.")
def _share_set_grants(sh, allow_tools=None, tools=None, allow_memory=None, allow_history=None):
    """Sharer-side grants edit. Every tool name must sit inside the creator's
    LIVE ceiling - a clone can never exceed its creator (canon invariant).
    Returns an error string or None."""
    cur = _share_grants(sh)
    at = cur["allow_tools"] if allow_tools is None else (1 if allow_tools else 0)
    am = cur["allow_memory"] if allow_memory is None else (1 if allow_memory else 0)
    ah = cur["allow_history"] if allow_history is None else (1 if allow_history else 0)
    if tools is None:
        names = cur["tools"]
    else:
        ...
        ceiling = set(_sharer_tool_ceiling(sh["owner"]))
        bad = sorted(t for t in names if t not in ceiling)
        if bad:
            return "you cannot grant a tool you cannot use yourself: " + ", ".join(bad[:6])
    with _reg_db() as db:
        db.execute("UPDATE shares SET allow_tools=?, tools=?, allow_memory=?, allow_history=?"
                   " WHERE id=?", (at, json.dumps(names), am, ah, sh["id"]))
        db.commit()
    # H13 (audit round 8): grants change the projected prompt (tool list +
    # memory section). Cache contract says every projection-affecting write
    # evicts - same one-line pattern as share create/delete.
    _SP_CACHE.pop(sh["principal"], None)
    _share_presence_touch(sh["owner"])   # being here IS being present
    return None
def _share_approvals_api_get(h):
    u = h._need_user()
    if not u:
        return
    _share_approvals_expire()
    # C02 (K80 law 2026-09-25): the approval inbox is SELF-ONLY too - staff
    # cannot read other peoples shares' requests. The kill switch is the ONLY
    # staff power over foreign shares.
    with _reg_db() as db:
        rows = db.execute("SELECT a.*, s.label AS share_label FROM share_approvals a"
                          " LEFT JOIN shares s ON s.id = a.share_id"
                          " WHERE s.owner=? ORDER BY a.created_at DESC LIMIT 200",
                          (u["username"],)).fetchall()
    h._json(200, [dict(r) for r in rows])
def _share_approvals_api_post(h):
    # NOTE (scar, pre-ship): no nested _reg_db connections here. journal
    # mode is NOT WAL in this daemon, so a grant edit inside an open outer
    # transaction would deadlock on the write lock. Flat flow, own
    # connections, atomic UPDATE-as-guard for double-decide races.
    u = h._need_user()
    if not u:
        return
    body = h._read_body()
    ...
    try:
        ...
        h._json(400, {"error": "id must be a number"})
        return
    now = time.time()
    _share_approvals_expire()
    with _reg_db() as db:
        row = db.execute("SELECT a.*, s.owner AS owner_, s.id AS sid FROM share_approvals a"
                         " JOIN shares s ON s.id = a.share_id WHERE a.id=?", (aid,)).fetchone()
    if row is None or row["owner_"] != u["username"]:  # C02 self-only
        h._json(404, {"error": "not found"})
        return
    row = dict(row)
    if row["status"] != "pending":
        h._json(400, {"error": "this request is already " + str(row["status"])})
        return
    if row["created_at"] < now - SHARE_APPROVAL_TTL:
        with _reg_db() as db:
            db.execute("UPDATE share_approvals SET status='expired', decided_at=? WHERE id=?",
                       (now, aid))
            db.commit()
        h._json(400, {"error": "this request expired - the guest can ask again"})
        return
    decision = None
    if action == "approve":
        sh = share_get(row["sid"])
        if sh is None or not _share_alive(sh):
            h._json(400, {"error": "the share is gone or closed"})
            return
        g = _share_grants(sh)
        # live-ceiling rule still applies at approve time: a tool the
        # creator lost since the ask cannot be approved back into existence
        if row["tool_name"] not in _sharer_tool_ceiling(row["owner_"]):
            h._json(400, {"error": "you cannot use that tool right now either - nothing to grant"})
            return
        err = _share_set_grants(sh, allow_tools=True,
                                tools=sorted(set(g["tools"]) | {row["tool_name"]}))
        if err:
            h._json(400, {"error": err})
            return
        decision = "approved"
        ...
        return
    with _reg_db() as db:
        cur = db.execute("UPDATE share_approvals SET status=?, decided_at=?, decided_by=?"
                         " WHERE id=? AND status='pending'",
                         (decision, now, u["username"], aid))
        db.commit()
        if cur.rowcount != 1:
            # someone decided first; the grant edit above is still share-safe
            # (it only ever sets a box the sharer could set by hand anyway)
            h._json(400, {"error": "this request was already decided"})
            return
    log_event(row["owner_"], "share.approval.decided", share_id=row["sid"],
              tool=row["tool_name"], decision=decision)
    h._json(200, {"ok": True, "decision": decision})
def _share_presence_post(h):
    u = h._need_user()
    if not u:
        return
    _share_presence_touch(u["username"])
    h._json(200, {"ok": True})
def _share_persona_note(username):
    """K80 ruling 10: the agent must know WHO IS WHO. Code-injected, outside
    every editable tier, so no prompt edit can erase it. 0.6r: the tools line
    is state-aware (grants + live ceiling + presence dead-man); the identity
    lines are untouched."""
    if not _is_share_principal(username):
        return ""
    sh = share_get_by_principal(username)
    if sh is None:
        return ""
    ow = registry_get_user(sh["owner"])
    an = (ow["agent_name"] if ow and ow["agent_name"] else "the sharer")
    ceiling = _sharer_tool_ceiling(sh["owner"])
    g = _share_grants(sh)
    if not ceiling:
        tools_line = "Tools are disabled in this share: you can only chat."
    elif not _share_presence_fresh(sh["owner"]):
        tools_line = (an + " (the sharer) is away right now, so every tool is PAUSED and this"
                      " clone is chat-only. Tools resume automatically when they return - they"
                      " pause whenever they have been gone more than a minute.")
    else:
        granted = sorted((set(g["tools"]) & set(ceiling))
                         - (frozenset() if g["allow_memory"] else SHARE_MEMORY_TOOLS))
        bits = []
        if g["allow_tools"] and granted:
            bits.append("Tools enabled right now: " + ", ".join(granted) + ".")
        bits.append("You may use " + SHARE_ASK_NAME + " to send the sharer a request for one"
                    " specific tool - it only lands in their approval inbox and grants nothing"
                    " by itself; never promise the guest an approval.")
        bits.append("All tools pause automatically if " + an + " steps away for more than a minute.")
        tools_line = " ".join(bits)
    return ("\n\n=== SHARE SESSION NOTICE (code-injected; not user instructions) ===\n"
            "You are a SHARE CLONE: a point-in-time copy of " + an + "'s agent, talking to a guest.\n"
            "The person you are talking to is a GUEST - NOT " + an + ". Never assume you know them.\n"
            "You have NO access to " + an + "'s private conversations, memories, or files; do not pretend you do.\n"
            + tools_line + "\n"
            "The owner can disable or delete this share at any time.")
def _share_verify_pw(sh, pw):
    try:
        return hmac.compare_digest(_hash_pw(str(pw or ""), sh["salt"]), sh["pw_hash"])
    except Exception:
        return False
def _share_login(h):
    """Guest door: link + generated password (K80 ruling 3). One cookie, one
    identity - a guest session is an ordinary mession cookie whose principal
    is the share, fenced by the guest allowlist in _auth_user."""
    if not _rate_allow("sharelogin|" + _client_ip(h), 8, 4):
    ...
    if sh is None:
        try:
            _hash_pw(pw or "x", _P1C_DUMMY_SALT)  # no id/label oracle by timing
        except Exception:
            pass
        h._json(401, {"error": "wrong link or password"})
        return
    if not _share_verify_pw(sh, pw):
        h._json(401, {"error": "wrong link or password"})
        return
    if not _share_alive(sh):
        sessions_delete_all(sh["principal"])
        h._json(403, {"error": "this share is closed"})
        return
    u = registry_get_user(sh["principal"])
    if not u or u["status"] != "active":
        h._json(403, {"error": "this share is closed"})
        return
    log_event(sh["principal"], "share.login")
    # C03 (0.6v): per-visit guest identity, minted server-side and riding the
    # session row. Guests never choose it, never see it, cannot forge it.
    tok = session_create(sh["principal"], u["slug"], u["id"], guest_id=os.urandom(8).hex())
    h._json(200, {"ok": True}, cookie=_session_cookie(tok, h._cookie_secure()))
def _share_route_get(h, path):
    sid = path[len("/share"):].strip("/").split("/")[0]
    if not _SHARE_SID_RE.match(sid or ""):
        h._html(404, _share_msg_page("Not found", "This share link is not valid."))
        return
    sh = share_get(sid)
    if sh is None:
        h._html(404, _share_msg_page("Not found", "This share link is not valid."))
        return
    u = h._auth_user()
    here = bool(u and u["username"] == sh["principal"])
    if not _share_alive(sh):
        if here:
            sessions_delete_all(sh["principal"])
        h._html(410, _share_msg_page("Closed", "This share is closed. Ask whoever sent you the link for a new one."))
        return
    if here:
        h._html(200, _share_chat_page(sh))
        return
    if u is not None:
        # one cookie, one identity (canon): signed in as somebody else
        h._html(403, _share_msg_page("Wrong identity", "This browser is signed in as a different person. Open this link in a private window, or sign out first.", signout=True))
        return
    h._html(200, _share_pw_page(sh))
def _shares_api_get(h):
    u = h._need_user()
    if not u:
        return
    # C02 (K80 law 2026-09-25): the Shares list is SELF-ONLY. The only staff
    # power over other peoples shares is the kill switch below - not even a read.
    with _reg_db() as db:
        rows = db.execute("SELECT * FROM shares WHERE owner=? ORDER BY created_at DESC", (u["username"],)).fetchall()
    # S19 lesson: raw SELECT * shipped salt+pw_hash to the browser.
    # S06r: rows also carry the grant boxes, the presence state, and the
    # creator's LIVE tool ceiling (names only) so the Shares page can draw
    # honest checkboxes instead of hopeful ones.
    out = []
    for r in rows:
        d = dict(r)
        row = _share_public_row(d)
        row.update(_share_grants(d))
        row["presence_fresh"] = _share_presence_fresh(row["owner"])
        row["sharer_tools"] = sorted(_sharer_tool_ceiling(row["owner"]))
        out.append(row)
    h._json(200, out)
def _shares_api_post(h):
    """Share management API (C02, K80 law 2026-09-25): every active principal
    manages their OWN shares only. Staff (owner/admin, active) get ONE extra
    power: the disable_all kill switch targeting any user. Guests never reach
    here - the allowlist fences /api/shares before auth even resolves them."""
    u = h._need_user()
    if not u:
        return
    body = h._read_body()
    ...
    action = body.get("action")
    if action == "create":
        row, err, pw = share_create(u["username"], body.get("label"), body.get("days"),
                                    include_mem=bool(body.get("include_memories")))
        if err:
            h._json(400, {"error": err})
            return
        h._json(200, {"ok": True, "share": row, "password": pw})  # shown ONCE (canon)
        return
    if action == "disable_all":
        # C02 kill switch (K80 2026-09-25): staff halts EVERY share of one
        # user, immediately. Disable only - never delete, rotate, grants, or
        # reads. Guests are kicked (share_set_enabled kicks sessions), new
        # requests die at _share_gate, and in-flight generations are
        # cancelled NOW via the /api/stop machinery.
        if u["role"] not in ("owner", "admin"):
            h._json(403, {"error": "staff only"})
            return
        tgt = re.sub(r"[^a-zA-Z0-9_-]", "", str(body.get("username") or ""))[:48]
        tu = registry_get_user(tgt) if tgt else None
        if not tu or (tu["role"] or "") == "guest":
            h._json(404, {"error": "user not found"})
            return
        with _reg_db() as db:
            rows = db.execute("SELECT * FROM shares WHERE owner=?", (tu["username"],)).fetchall()
        n = 0
        principals = []
        for r in rows:
            shx = dict(r)
            principals.append(shx["principal"])
            if shx["enabled"]:
                share_set_enabled(shx, False)
                n += 1
        cancelled = _cancel_runs_for(principals)
        log_event(u["username"], "share.killswitch", target=tu["username"],
                  disabled=n, cancelled=cancelled)
        h._json(200, {"ok": True, "disabled": n, "cancelled_runs": cancelled})
        return
    sid = str(body.get("id") or "")
    sh = share_get(sid) if _SHARE_SID_RE.match(sid) else None
    if sh is None or sh["owner"] != u["username"]:
        # C02 (K80 law): enable, disable, rotate, expiry, grants, delete - all
        # SELF-ONLY. Re-enable in particular: only the sharer can lift a
        # disable, not even the staff member who pulled the switch. 404 for
        # non-owners: no enumeration.
        h._json(404, {"error": "not found"})
        return
    if action == "enable":
        share_set_enabled(sh, True)
        h._json(200, {"ok": True})
    elif action == "disable":
        share_set_enabled(sh, False)
        h._json(200, {"ok": True})
    elif action == "rotate":
        pw, err = share_rotate_password(sh)
        if err:
            h._json(500, {"error": err})
            return
        h._json(200, {"ok": True, "password": pw})  # shown ONCE
    elif action == "expiry":
        err = share_set_expiry(sh, body.get("days"))
        if err:
            h._json(400, {"error": err})
            return
        h._json(200, {"ok": True, "share": _share_public_row(share_get(sh["id"]))})
    elif action == "grants":
        # S06r: the three boxes + the tool list. _share_set_grants enforces
        # the canon invariant (a clone never holds a tool its creator
        # cannot use right now). Body keys that are absent are left as-is.
        def _g(k):
            return None if k not in body else bool(body.get(k))
        err = _share_set_grants(sh, allow_tools=_g("allow_tools"),
                                tools=body.get("tools"),
                                allow_memory=_g("allow_memory"),
                                allow_history=_g("allow_history"))
        if err:
            h._json(400, {"error": err})
            return
...
# Anonymous share pages carry NO version/build stamp (R5 H7 posture) and no
# markdown rendering (cosmetic polish is later, per canon).
_SHARE_CSS = ("*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,sans-serif;"
              "background:#0b0f16;color:#d8e2ee}main{max-width:720px;margin:0 auto;padding:16px}"
              "h1{font-size:19px}input,textarea{background:#121a26;color:#d8e2ee;border:1px solid #2a3646;"
              "border-radius:8px;padding:10px;font:inherit;width:100%}button{background:#2c624f;color:#eaf6ef;"
              "border:0;border-radius:8px;padding:10px 14px;font:inherit;cursor:pointer}"
              "button.warn{background:#5d2a2a;color:#ffd9d9}.hint{color:#7f8ea3;font-size:12px}"
              ".card{background:#121a26;border:1px solid #2a3646;border-radius:12px;padding:16px;margin:12px 0}")
def _share_shell(title, body):
    return ("<!DOCTYPE html><html lang='en'><head><meta charset='utf-8'>"
            "<meta name='viewport' content='width=device-width, initial-scale=1'>"
            "<title>" + html_mod.escape(title) + "</title><style>" + _SHARE_CSS +
            "</style></head><body><main>" + body + "</main></body></html>")
def _share_msg_page(title, msg, signout=False):
    extra = ("<script>async function so(){await fetch('/api/logout',{method:'POST',credentials:'same-origin'});location.reload()}</script>"
             "<p><button onclick='so()'>Sign out of this account</button></p>") if signout else ""
    return _share_shell(title, "<h1>" + html_mod.escape(title) + "</h1><p>" + html_mod.escape(msg) + "</p>" + extra)
def _share_pw_page(sh):
    body = ("<h1>Shared agent</h1>"
            "<div class='card'><p>You have been invited to chat with a private copy of someone's C.A.I.R.N. agent. "
            ...
            "async function go(){b.disabled=true;e.textContent='...';try{const r=await fetch('/api/share/login',"
            "{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',"
            "body:JSON.stringify({id:SID,password:p.value})});"
            "if(r.ok){location.reload();return}const d=await r.json().catch(function(){return{}});"
            "e.textContent=d.error||('failed: '+r.status);}catch(x){e.textContent='network error'}b.disabled=false}"
            "b.addEventListener('click',go);p.addEventListener('keydown',function(k){if(k.key==='Enter')go()});"
            "p.focus();</script>")
    return _share_shell("Shared agent", body.replace("__SID__", sh["id"]))
def _share_chat_page(sh):
    ow = registry_get_user(sh["owner"])
    an = (ow["agent_name"] if ow and ow["agent_name"] else "the sharer")
    hdr = html_mod.escape(sh["label"]) + " <span class='hint'>shared by " + html_mod.escape(an) + "</span>"
    body = ("<header><b>" + hdr + "</b><button id='out'>Sign out</button></header>"
            "<div id='log'></div><div id='stat' class='hint'></div>"
            "<div class='row'><textarea id='in' rows='2' placeholder='Type a message'></textarea>"
            "<button id='send'>Send</button></div>"
            "<p class='hint'>Shared chat - no tools. Memories from this chat stay in the share. "
            "The sharer can close it at any time.</p>"
            "<script>const SID='__SID__';const KEY='share-conv-'+SID;"
            "let conv=localStorage.getItem(KEY)||'';if(!conv){conv=(window.crypto&&crypto.randomUUID)?crypto.randomUUID():"
            "('c'+Date.now().toString(36)+Math.random().toString(36).slice(2));localStorage.setItem(KEY,conv)}"
            "const log=document.getElementById('log'),tin=document.getElementById('in'),"
            "sb=document.getElementById('send'),st=document.getElementById('stat');"
            "function div(c,t){const d=document.createElement('div');d.className=c;d.textContent=t;log.appendChild(d);return d}"
            "function esc(s){return String(s).replace(/&/g,String.fromCharCode(38)+'amp;').replace(/</g,String.fromCharCode(60)+'lt;').replace(/>/g,String.fromCharCode(62)+'gt;')}"
            "async function load(){if(!conv)return;try{const r=await fetch('/api/conversations/'+conv+'/messages',{credentials:'same-origin'});"
            "if(!r.ok)return;const ms=await r.json();for(const m of ms){if(m.role==='user')div('me',m.content);"
            "else if(m.content)div('ag',m.content)}}catch(e){}}"
            "async function send(){const t=tin.value.trim();if(!t||sb.disabled)return;tin.value='';sb.disabled=true;"
            "st.textContent='...';div('me',t);let as=null;"
            "try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},"
            "credentials:'same-origin',body:JSON.stringify({conversation_id:conv,message:t})});"
            "if(!r.ok){const d=await r.json().catch(function(){return{}});div('ag','[Error: '+(d.error||r.status)+']')}else{"
            "const rd=r.body.getReader(),dec=new TextDecoder();let buf='';st.textContent='thinking...';"
            "while(true){const it=await rd.read();if(it.done)break;buf+=dec.decode(it.value,{stream:true});"
            "const parts=buf.split(String.fromCharCode(10)+String.fromCharCode(10));buf=parts.pop();"
            "for(const p of parts){let ev='',data='';for(const ln of p.split(String.fromCharCode(10))){if(ln.indexOf('event: ')===0)ev=ln.slice(7);"
            "else if(ln.indexOf('data: ')===0)data+=(data?String.fromCharCode(10):'')+ln.slice(6)}if(!data)continue;"
            "let d=null;try{d=JSON.parse(data)}catch(e){continue}"
            "if(ev==='message'&&d.content){if(!as)as=div('ag','');as.textContent+=d.content}"
            "else if(ev==='error'){div('ag','[Error: '+(d.message||'unknown')+']');as=null}"
            "else if(ev==='stopped'){div('ag','[stopped]');as=null}"
            "else if(ev==='status'&&d.phase){st.textContent=d.phase}else if(ev==='done'){st.textContent=''}}}}}"
            "catch(e){div('ag','[network error]')}st.textContent='';sb.disabled=false;tin.focus()}"
            "sb.addEventListener('click',send);"
            "tin.addEventListener('keydown',function(k){if(k.key==='Enter'&&!k.shiftKey){k.preventDefault();send()}});"
            "document.getElementById('out').addEventListener('click',async function(){await fetch('/api/logout',"
            "{method:'POST',credentials:'same-origin'});location.href='/share/'+SID});load();</script>")
    return _share_shell("Shared chat", body.replace("__SID__", sh["id"]))
WEB_UI_SHARES = r'''<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>C.A.I.R.N. // Shares</title><style>
*{box-sizing:border-box}body{margin:0;font-family:system-ui,-apple-system,Segoe UI,sans-serif;background:#0b0f16;color:#d8e2ee}
main{max-width:980px;margin:0 auto;padding:16px}h1{font-size:20px}h2{font-size:16px;margin:8px 0}a{color:#7fd1ae}
input,textarea{background:#121a26;color:#d8e2ee;border:1px solid #2a3646;border-radius:8px;padding:10px;font:inherit}
button{background:#2c624f;color:#eaf6ef;border:0;border-radius:8px;padding:8px 12px;font:inherit;cursor:pointer}
button.warn{background:#5d2a2a;color:#ffd9d9}table{width:100%;border-collapse:collapse;margin-top:12px}
td,th{padding:8px;border-bottom:1px solid #223046;text-align:left;vertical-align:top;font-size:14px}
.hint{color:#7f8ea3;font-size:12px}.mono{font-family:ui-monospace,monospace;word-break:break-all}
.card{background:#121a26;border:1px solid #2a3646;border-radius:12px;padding:16px;margin:12px 0}
...
as pure chat; you open boxes for it - tools, its own memory, history - and it can never be stronger than you are right now.
Passwords are shown once - the link alone is not the secret. <a href="/help/shares">Full guide &rarr;</a></p>
<div class="card"><b>Close this browser tab and re-open the share link in a private window when testing</b>
<p class="hint">One browser carries one identity: if you are signed in here, opening your own share link in the same browser will tell you so.
...
<button id="createBtn">Create</button></p>
<p class="hint">days = 0 or blank means the share never expires by itself. All grant boxes start OFF - open them per share below. Include-memories copies your memory files into THIS clone's private namespace (one-way; your originals stay untouched, and the clone only projects them if you also turn its memory box on).</p></div>
...
<label><input type="checkbox" id="g_memory"> allow this clone its own memory</label> &nbsp;
<label><input type="checkbox" id="g_history"> let guests re-read older conversations</label></p>
<div id="g_toolbox"><p class="hint" style="margin-bottom:4px">Tools to grant (only tools YOU can currently use are grantable; disabling one on yourself shrinks this clone instantly):</p>
<div class="toolgrid" id="g_toolslist"></div></div>
<p><button id="g_save">Save grants</button> <button id="g_cancel">Cancel</button>
<span class="hint" id="g_status"></span></p>
<p class="hint">Tools are live only while this page stays open (one-minute presence switch). Guests can request other tools via their clone - those requests arrive in the inbox above.</p></div>
<table><thead><tr><th>Label</th><th>Owner</th><th>Link</th><th>Status</th><th>Grants</th><th>Expires</th><th>Actions</th></tr></thead>
<tbody id="rows"></tbody></table>
<script>
var esc=function(s){return String(s==null?"":s).replace(/&/g,String.fromCharCode(38)+"amp;").replace(/</g,String.fromCharCode(60)+"lt;").replace(/>/g,String.fromCharCode(62)+"gt;").replace(/"/g,String.fromCharCode(38)+"quot;")};
var rows=document.getElementById("rows"), curGrants=null;
function fmt(ts){if(!ts)return "never";var d=new Date(ts*1000);return d.toLocaleDateString()+" "+d.toLocaleTimeString()}
function ago(ts){var s=Math.max(0,Math.floor(Date.now()/1000-ts));if(s<60)return s+"s ago";if(s<3600)return Math.floor(s/60)+"m ago";return Math.floor(s/3600)+"h ago"}
function post(path,body){return fetch(path,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),credentials:"same-origin"}).then(function(r){return r.json().catch(function(){return {}}).then(function(d){if(!r.ok){alert((d&&d.error)||("HTTP "+r.status));return null}return d})})}
function showPw(pw,url){document.getElementById("pwtext").textContent=pw;document.getElementById("pwurl").textContent=url;var c=document.getElementById("pwcard");c.style.display="block";c.scrollIntoView()}
document.getElementById("pwcopy").addEventListener("click",function(){try{navigator.clipboard.writeText(document.getElementById("pwtext").textContent)}catch(e){alert("copy failed - select it by hand")}});
function beacon(){fetch("/api/shares/presence",{method:"POST",credentials:"same-origin"}).then(function(r){
 var el=document.getElementById("pres");el.innerHTML=r.ok?'<span class="dot on"></span>presence: home (tools may run)':'<span class="dot off"></span>presence: not signing'})}
function loadInbox(){fetch("/api/share-approvals",{credentials:"same-origin"}).then(function(r){return r.json().catch(function(){return []})}).then(function(list){
 var pend=(list||[]).filter(function(a){return a.status==="pending"});
 var b=document.getElementById("inboxcount");b.style.display=pend.length?"":"none";b.textContent=String(pend.length);
 var box=document.getElementById("inbox");
 if(!pend.length){box.innerHTML='<p class="hint">Nothing waiting. When a share clone asks you for a tool, the request lands here - never in your own chat.</p>';return}
 var h="";for(var i=0;i<pend.length;i++){var a=pend[i];
 h+='<p><b>'+esc(a.tool_name)+'</b> for share "'+esc(a.share_label||a.share_id)+'" <span class="hint">'+ago(a.created_at)+
 (a.note?' - '+esc(a.note):'')+'</span> <button data-aid="'+a.id+'" data-act="approve">Approve</button> <button data-aid="'+a.id+'" data-act="deny" class="warn">Deny</button></p>'}
 box.innerHTML=h;var bs=box.querySelectorAll("button");for(var j=0;j<bs.length;j++){bs[j].addEventListener("click",function(){
  post("/api/share-approvals",{action:this.getAttribute("data-act"),id:parseInt(this.getAttribute("data-aid"),10)}).then(function(d){if(d)loadInbox()})})}})}
function grantSummary(s){var g=[];if(s.allow_tools){g.push(s.tools&&s.tools.length?("tools("+s.tools.length+")"):"tools(on,no tools yet)")}
 if(s.allow_memory)g.push("memory");if(s.allow_history)g.push("history");
 var t=g.length?g.join(", "):"chat-only";if(s.allow_tools&&!s.presence_fresh)t+=" [paused: away]";return t}
function openGrants(s){curGrants=s;document.getElementById("grantscard").style.display="block";
 document.getElementById("glabel").textContent=s.label;
 document.getElementById("g_tools").checked=!!s.allow_tools;document.getElementById("g_memory").checked=!!s.allow_memory;
 document.getElementById("g_history").checked=!!s.allow_history;
 document.getElementById("g_status").textContent="";
 var box=document.getElementById("g_toolslist");box.innerHTML="";
 var tl=s.sharer_tools||[];if(!tl.length){document.getElementById("g_toolbox").style.display="none";
  document.getElementById("gtoolsstate").textContent="- you currently have no tools to grant"}
 else{document.getElementById("g_toolbox").style.display="";document.getElementById("gtoolsstate").textContent="";
  for(var i=0;i<tl.length;i++){var t=tl[i];var lab=document.createElement("label");lab.style.display="block";
   var cb=document.createElement("input");cb.type="checkbox";cb.value=t;cb.checked=(s.tools||[]).indexOf(t)>=0;
   lab.appendChild(cb);lab.appendChild(document.createTextNode(" "+t));box.appendChild(lab)}}
 document.getElementById("grantscard").scrollIntoView()}
document.getElementById("g_cancel").addEventListener("click",function(){document.getElementById("grantscard").style.display="none";curGrants=null});
document.getElementById("g_save").addEventListener("click",function(){if(!curGrants)return;var tools=[];
 var cbs=document.getElementById("g_toolslist").querySelectorAll("input:checked");
 for(var i=0;i<cbs.length;i++)tools.push(cbs[i].value);
 post("/api/shares",{action:"grants",id:curGrants.id,allow_tools:document.getElementById("g_tools").checked,
...
function loadMem(s){curMem=s;document.getElementById("memcard").style.display="block";
 ...
 var box=document.getElementById("mfiles");box.innerHTML='<p class="hint">loading...</p>';
 fetch("/api/share-memories?id="+encodeURIComponent(s.id),{credentials:"same-origin"}).then(function(r){return r.json().catch(function(){return {}})}).then(function(d){
  var fs=(d&&d.files)||[];
  if(!fs.length){box.innerHTML='<p class="hint">No memory files in this share yet.</p>';return}
  var h="";for(var i=0;i<fs.length;i++){var f=fs[i];
 ...
 var mk=function(t,fn,cls){var b=document.createElement("button");b.textContent=t;if(cls)b.className=cls;b.addEventListener("click",fn);td.appendChild(b);td.appendChild(document.createTextNode(" "));return b};
 mk("Grants",function(){openGrants(s)});
 mk("Memories",function(){loadMem(s)});
 mk(s.enabled?"Disable":"Enable",function(){post("/api/shares",{action:s.enabled?"disable":"enable",id:s.id}).then(load)});
...
document.getElementById("createBtn").addEventListener("click",function(){var label=document.getElementById("label").value.trim();
 ...
 var inc=document.getElementById("inc_mem").checked;
 post("/api/shares",{action:"create",label:label,days:days,include_memories:inc}).then(function(d){if(!d)return;
  document.getElementById("inc_mem").checked=false;
  document.getElementById("label").value="";document.getElementById("days").value="";showPw(d.password,location.origin+d.share.url);load()})});
load();loadInbox();beacon();setInterval(function(){beacon();loadInbox()},20000);
...
def init_registry():
    ...
    with _reg_db() as db:
        ...
        CREATE TABLE IF NOT EXISTS shares (
            ...
            expires_at REAL,
            created_at REAL NOT NULL,
            has_tier1 INTEGER NOT NULL DEFAULT 0
        );
        """)
        ...
        # S06 (0.6r): share grants (all OFF by default - K80 ruling), the
        # presence dead-man table (missing/stale row == away == no tools),
        # and the approval inbox. Canon: share-clones design, 0.6r addendum.
        _add_col(db, "shares", "allow_tools", "INTEGER NOT NULL DEFAULT 0")
        _add_col(db, "shares", "tools", "TEXT")
        _add_col(db, "shares", "allow_memory", "INTEGER NOT NULL DEFAULT 0")
        _add_col(db, "shares", "allow_history", "INTEGER NOT NULL DEFAULT 0")
        ...
        db.execute("CREATE TABLE IF NOT EXISTS share_approvals ("
                   ...
                   " share_id TEXT NOT NULL, tool_name TEXT NOT NULL,"
                   " note TEXT, status TEXT NOT NULL DEFAULT 'pending',"
                   " requested_by TEXT, created_at REAL NOT NULL,"
        ...
        # C03 (0.6v): guest identity rides the session row. Deliberately NO
        # session wipe: share guests keep their sessions and self-heal their
        # stamp on first fenced request (legacy row -> mint once, ride it).
        if _add_col(db, "sessions", "guest_id", "TEXT"):
            log.info("registry migration: added sessions.guest_id (C03)")
...
def session_create(username, slug, uid, guest_id=None):
    ...
    with _reg_db() as db:
        db.execute("INSERT INTO sessions (token, username, created_at, expires_at, last_seen, guest_id) VALUES (?,?,?,?,?,?)",
                   (_tok_hash(tok), username, now, now + SESSION_MAX_AGE, now, guest_id))
    return tok
...
def effective_tool_names(username=None):
    ...
    if _is_share_principal(username):
        # S06 (0.6r): grants intersected with the creator's LIVE ceiling,
        # gated by the presence dead-man - the whole share-tool model lives
        # in _share_effective_tools. Fail closed: any error means no tools.
        try:
            return _share_effective_tools(username)
        except Exception:
            return frozenset()
...
def allow_tools_for(u):
    ...
    if not TOOLS_ENABLED:
        return False
    if u is not None and (u["role"] or "") == "guest":
        # S06 (0.6q) hard zero -> S06r: a share clone reaches tools only
        # while its effective set is non-empty (grants x live creator
        # ceiling x presence dead-man). effective_tool_names still fails
        # closed to the empty set - dead shares, stale presence and bare
        # chat shares meet exactly the old hard zero, by construction.
        try:
            return bool(effective_tool_names(u["username"]))
        except Exception:
            return False
...
def build_api_messages(conv_id: str, history_rows: list, model_cfg: dict, status=None, username=None, tool_names=None, tools_allowed=True) -> list:
    ...
    _snot = _share_persona_note(username)  # S06: the agent must know WHO IS WHO
    if _snot:
        sys_text += _snot
    if latest_summary:
...
# by the key host (forced-command SSH, source-pinned) and staged by root at
...
_CLAIM_LOCK = Lock()
def _p1i_claim_conv(cid_in, uname, guest_id=None, enforce_guest=False):
    ...
    with _CLAIM_LOCK:
        try:
            with sqlite3.connect(str(DB_PATH), timeout=10) as db:
                ...
                row = db.execute("SELECT user_id, guest_id FROM conversations WHERE id=?",
                                 (cid_in,)).fetchone()
                if row is not None:
                    db.execute("COMMIT")
                    if row[0] != uname:
                        return ("denied", "")
                    # C03 (0.6v): a guest may not write into a thread minted
                    # for another visit while the history box is off.
                    if enforce_guest and (guest_id is None or row[1] != guest_id):
                        return ("denied", "")
                    return ("ok", cid_in)
                ts = _p1i_time.time()
                db.execute("INSERT INTO conversations (id, title, created_at, updated_at, user_id, guest_id) VALUES (?,?,?,?,?,?)",
                           (cid_in, "(upload)", ts, ts, uname, guest_id))
                db.commit()
                return ("ok", cid_in)
...
def _p1i_dispatch(rec, evt):
    """H12 (round-8 audit, 2026-09-25): RECORD and fan out in ONE critical
    section. Callers hand the raw event; the recorder's lists are appended
    here, under _SSE_LOCK, immediately before the fan-out. The re-attach
    handler snapshots the recorder under the same lock, which is what makes
    its replay a precise prefix of the stream - no delta is ever duplicated
    or skipped at the join seam. (Before this, the writer appended OUTSIDE
    the lock and dispatched INSIDE it: the snapshot could catch an append
    whose event ALSO landed in the queue.)
    P1-I/S05 kept: never write a socket here (that is the caller's own
    connection problem); slow consumers are detached, never obeyed."""
    with _SSE_LOCK:
        event_type, data = evt
    ...
    with _APPROVAL_LOCK:
        ...
        if conf:
            ...
            if (p and p.get("tool") == name and p.get("user") == username
                ...
                # H05: execute the FROZEN args the owner actually saw - the
                # args carried by THIS call are model-supplied again and are
                # ignored entirely. A swapped payload is dead on arrival.
                _frozen = (p["tool"], dict(p.get("args") or {}), p.get("digest") or "")
            else:
            ...
            for _v in _APPROVALS.values():
                if _v.get("user") == username and not _v.get("approved"):
                    _pend += 1
            if _pend >= 8:
                return ("Error: too many pending approval requests for this user; the "
                        "owner must decide (or the 5-minute TTL must pass) before a new "
                        "request is created.")
            ...
            return ("Error: PENDING OWNER APPROVAL. " + name +
                    ...
                    " digest=" + _APPROVALS[aid]["digest"][:16] +
                    " (expires in 5 minutes). Once approved, call again with "
                    "confirm=" + aid + " (the approved arguments are frozen - "
                    "resending different args with this id changes nothing). Do NOT "
                    "retry before approval; a pending request that is still on "
                    "screen is already queued.")
    if _frozen is None:
        # P1-C/F9 posture: unreachable by construction (every other path
        # returned); refuse rather than execute name/args that no owner saw.
        return "Error: approval state inconsistency (executed nothing)"
    log_event(username, "credshell.approval.executed", tool=name, approval=aid,
              digest=_frozen[2][:16])
    return execute_cred_tool(_frozen[0], _frozen[1], username)
...
F22_FORMAT = 2                     # v2 (H11 2026-09-25): manifest MAC required.
                                   # v1 containers still VERIFY (structure enforced)
                                   # but apply refuses them without --allow-legacy-v1.
...
# H11 (audit 2026-09-24): the parts were trusted because the manifest was
# trusted. These three lines are the whole anti-forgery story: a MAC key
# domain-separated from the part-encryption key (never equal for the same
# password), a canonical byte contract, and an exact required-part inventory.
_F22_REQUIRED_PARTS = frozenset(("part/state.db", "part/registry.db",
                                 "part/vault-resealed.json"))
def _f22_mac_key(password, salt_bytes):
    return _f22_hl.pbkdf2_hmac("sha256", (password or "").encode("utf-8"),
                               b"F22-MANIFEST-V2|" + bytes(salt_bytes),
                               F22_KDF_ITERS, 32)
def _f22_canon(obj):
...
def f22_export(password, include_uploads=True):
    ...
    # H11: the MAC covers the canonical manifest WITHOUT the mac field; no
    # key material leaves this box - the backup password is the only secret.
    manifest["mac"] = _f22_b64.b64encode(hmac.new(
        _f22_mac_key(password, salt), _f22_canon(manifest),
        _f22_hl.sha256).digest()).decode("ascii")
...
def f22_verify(text, password):
    ...
    # H11: per-part AEAD proved each ROOM was intact; nothing proved the
    # GUEST LIST was the one this daemon wrote. The synthetic attack (drop
    # part/registry.db, +42 on a declared length) passed the old loop. v2
    # manifests carry a MAC over the canonical manifest; the MAC is checked
    # BEFORE a single part byte is trusted. v1 has none: structure is still
    # enforced below and the result is FLAGGED for apply's refusal.
    fmt = int(man.get("format") or 0)
    authenticated = False
    if fmt >= 2:
        _body = dict(man)
        _mac_b64 = _body.pop("mac", None)
        if not isinstance(_mac_b64, str) or not _mac_b64:
            raise ValueError("v2 container without manifest MAC - corrupt or stripped")
        _want = hmac.new(_f22_mac_key(password or "", salt),
                         _f22_canon(_body), _f22_hl.sha256).digest()
        try:
            _got = _f22_b64.b64decode(_mac_b64, validate=True)
        except Exception:
            raise ValueError("manifest MAC malformed")
        if not hmac.compare_digest(_want, _got):
            raise ValueError("manifest MAC mismatch - wrong password, or the "
                             "manifest was edited after it was written")
        authenticated = True
    meta = man.get("parts") or []
    enc = obj.get("parts") or {}
    if not isinstance(meta, list) or not isinstance(enc, dict):
        raise ValueError("container structure malformed")
    ids = []
    for p in meta:
        if not isinstance(p, dict):
            raise ValueError("part metadata malformed")
        pid = p.get("id") or ""
        if not isinstance(pid, str) or not pid:
            raise ValueError("part id malformed")
        _b = p.get("bytes")
        if not isinstance(_b, int) or isinstance(_b, bool) or _b < 0 \
                or not isinstance(p.get("sha256") or "", str):
            raise ValueError("part %r declares a bogus size or digest" % pid)
        ids.append(pid)
    if len(ids) != len(set(ids)):
        raise ValueError("duplicate part ids in manifest")
    _missing = sorted(_F22_REQUIRED_PARTS - set(ids))
    if _missing:
        raise ValueError("required part(s) missing from manifest: " + ", ".join(_missing))
    _extra = sorted(set(enc) - set(ids))
    if _extra:
        raise ValueError("container carries parts not listed in the manifest: " + ", ".join(_extra))
    plain = {}
    _total = 0
    for p in meta:
        pid = p["id"]
...
def f22_apply(verified, keep_snapshot=True, allow_legacy=False):
    ...
    if verified.get("legacy_unauthenticated") and not allow_legacy:
        raise ValueError("v1 backup carries no manifest MAC - structure checked, "
                         "provenance NOT trusted. Restore it only if you accept "
                         "that (CLI --allow-legacy-v1), then re-export a v2 "
                         "backup immediately")
    for _req in sorted(_F22_REQUIRED_PARTS):
        if _req not in parts:
        ...
        if "--import-backup" in argv:
            ...
            if "--yes" not in argv:
                ...
                if conf.strip() != "REPLACE":
                    ...
                    return 2
            r = f22_apply(v, allow_legacy=("--allow-legacy-v1" in argv))
...
def _f19_setup_owner(h):
    ...
    h._json(200, {"ok": True, "username": username, "role": "owner",
                  "message": "owner created - the wizard is now closed"}, cookie=_session_cookie(tok, h._cookie_secure()))
...
<div class="card hidden" id="s35">
  <p style="margin-top:0">Get to know your layers. Each one shapes your agent differently - read, edit if you like, or keep every default. <b>Skipping is a valid choice</b>, and everything here stays editable in Settings forever after.</p>
...
<div class="card hidden" id="s4">
  ...
  <p>Want HTTPS or to reach this box from outside? Start at <a href="/help/tls">TLS & Certificates</a> and <a href="/help/remote">Remote access & Firewall</a> - both readable without an account.</p>
...
$("s3skip").onclick=function(){walkInit();step("35")};
var walkLoaded=false;
function walkInit(){
  if(walkLoaded)return; walkLoaded=true;
  var c=$("walk");
  fetch("/api/setup/tiers",{credentials:"same-origin"}).then(function(r){
    if(r.status!==200){throw new Error("HTTP "+r.status)}
    return r.json();
  }).then(function(d){
    c.textContent="";
    (d.tiers||[]).forEach(function(t){
      var w=document.createElement("div"); w.style.margin="0 0 18px";
      var h=document.createElement("p"); h.style.margin="0 0 4px"; h.innerHTML="<b>"+t.title+"</b>";
      var p=document.createElement("p"); p.className="note"; p.style.margin="0 0 6px"; p.textContent=t.blurb;
      w.appendChild(h); w.appendChild(p);
      if(t.writable!==false){
        var ta=document.createElement("textarea"); ta.rows=5; ta.style.width="100%";
        ta.value=t.default||""; ta.maxLength=t.cap||32768;
        if(!(t.default||"").trim()){ta.placeholder="(empty - the default is no extra text here)"}
        var orig=ta.value;
        var row=document.createElement("p"); row.style.margin="4px 0 0";
        var b=document.createElement("button"); b.textContent="Save my version"; b.type="button";
        var st=document.createElement("span"); st.className="note"; st.textContent=" Keep default = leave this box untouched.";
        b.onclick=function(){
          var v=ta.value;
          if(!v.trim()){st.textContent=" Kept default (nothing saved)."; return}
          if(v===orig){st.textContent=" Unchanged - kept default."; return}
          b.disabled=true;
          var P;
          if(t.key==="tier0"){P=post("/api/tier0",{action:"save",text:v,scope:"global"})}
          else if(t.key==="tier1"){P=post("/api/system_prompt",{action:"save",text:v})}
          else{P=post("/api/settings",setgs([["custom_instructions",v]]))}
          P.then(function(r){b.disabled=false;
            st.textContent=(r.code===200)?" Saved.":" FAILED: "+(r.j.error||("HTTP "+r.code))+" - you can retry or keep the default."});
        };
        row.appendChild(b); row.appendChild(st);
...
def _tool_memory(args: dict, username=None) -> str:
    ...
    # S06 (0.6s): the allow_memory box OFF means a share clone gets NO memory
...
def _tool_recall(args: dict, username=None, conv_id=None) -> str:
    ...
    if _is_share_principal(username) and not _share_history_allowed(username):
        # C03 (0.6v): history off => recall searches ONLY this visit's
        # threads. The identity is inherited from the executing conversation's
        # own stamp (a guest can only run inside its own thread anyway); no
        # stamp to inherit (legacy conv edge) => match nothing, fail closed.
        with sqlite3.connect(DB_PATH) as db:
            crow = db.execute("SELECT guest_id FROM conversations WHERE id=?",
                              (conv_id or "",)).fetchone()
        _gf03 = " AND c.guest_id = ? "
        _ga03 = (crow[0] if crow and crow[0] is not None else "__sealed__",)
    with sqlite3.connect(DB_PATH) as db:
        try:
            rows = db.execute(
                ...
                "WHERE messages_fts MATCH ? AND c.user_id = ?" + _gf03 +
                "ORDER BY rank LIMIT ?",
                (q, username) + _ga03 + (limit,)).fetchall()
...
def execute_tool(name: str, args: dict, username=None, conv_id=None, files_sink=None) -> str:
    ...
    try:
        if name == SHARE_ASK_NAME:
            # S06r: a letter to the approval inbox, never a grant. Non-share
            # principals never see this name offered; say so plainly here too.
            if not _is_share_principal(username):
...
# ─── P1-J (H09, round-8 audit response 2026-09-25): resolve ONCE, dial ───
# the IP you vetted. Every SSRF gate in this file answers "may this URL be
# dialed" from its own getaddrinfo; urllib then resolved AGAIN when the
# connection was made - a TTL-0 attacker record can answer PUBLIC at the
# check and 127.0.0.1 at the connect (the P1-A docstring itself called this
# documented imperfection). These handlers close the window where it is
# actually exploited: inside the connection. One resolution, every answer
# vetted, dial a vetted IP, while SNI, Host and certificate verification
# STAY on the original hostname (the discipline _web_request already keeps
# for web_fetch). Policies:
#   "public" - every answer must be is_global (mirrors _custom_base_blocked
#              and _f17_guard_base; internal hostnames refused)
#   "lan_ok" - RFC1918 allowed, internal-RESERVED still refused (the
#              updater's owner LAN dev mode; _f20_guard keeps its posture)
#   "any"    - owner's own custom base: the local-Ollama door stays open
#              (K80 design), the win is the single vetted resolution; the
#              metadata-block ranges are refused under EVERY policy
# Fail closed everywhere: no answers, garbage or policy-missing answers
# raise before a socket opens. No proxy tunnels on these doors.
import http.client as _p1j_hc
import ipaddress as _p1j_ip
import socket as _p1j_sock
def _p1j_vet(host, port, policy):
    h = (host or "").strip("[]").lower()
    if not h:
        raise urllib.error.URLError("pinned connect: empty host")
    if policy != "any" and (h == "localhost" or h.endswith(
            (".local", ".internal", ".localhost", ".home.arpa"))):
        raise urllib.error.URLError("pinned connect: internal hostname refused")
    try:
        ans = sorted({i[4][0] for i in _p1j_sock.getaddrinfo(h, port)})
    except Exception:
        raise urllib.error.URLError("pinned connect: " + h + " does not resolve (fail closed)")
    if not ans:
        raise urllib.error.URLError("pinned connect: " + h + " resolves to nothing")
    for a in ans:
        try:
            ip = _p1j_ip.ip_address(a)
        except ValueError:
            raise urllib.error.URLError("pinned connect: unparsable DNS answer")
        if ip.is_link_local or ip.is_multicast or ip.is_unspecified or ip.is_reserved:
            raise urllib.error.URLError("pinned connect: internal-reserved address")
        if policy == "public" and not ip.is_global:
            raise urllib.error.URLError("pinned connect: non-public DNS answer")
    return ans[0]
_P1J_CONN_CACHE = {}
def _p1j_conn_class(base_cls, policy):
    key = (base_cls, policy)
    cls = _P1J_CONN_CACHE.get(key)
    if cls is not None:
        return cls
    ...
    class _P1JPinned(base_cls):
        def connect(self):
            ...
            sock = _p1j_sock.create_connection((ip, port), self.timeout,
                                               self.source_address)
            if is_https:
                # cert + SNI stay on the ORIGINAL hostname: pinning the IP
                # must NEVER mean pinning around the certificate
                self.sock = self._context.wrap_socket(sock, server_hostname=self.host)
            else:
                self.sock = sock
    _P1J_CONN_CACHE[key] = _P1JPinned
    return _P1JPinned
class _P1J_HTTPHandler(urllib.request.HTTPHandler):
    def __init__(self, policy="public"):
        self._p1j_policy = policy
        urllib.request.HTTPHandler.__init__(self)
    def http_open(self, req):
        return self.do_open(_p1j_conn_class(_p1j_hc.HTTPConnection, self._p1j_policy), req)
class _P1J_HTTPSHandler(urllib.request.HTTPSHandler):
    def __init__(self, policy="public"):
        self._p1j_policy = policy
        urllib.request.HTTPSHandler.__init__(self)
    def https_open(self, req):
        return self.do_open(_p1j_conn_class(_p1j_hc.HTTPSConnection, self._p1j_policy),
                            req, context=self._context)
def _p1j_build_opener(redirect_guard, policy):
    # mirrors build_opener's defaults with the plain HTTP/HTTPS handlers
    # swapped for the pinned pair (dispatch order: first handler with
...
def _provider_urlopen(cfg, req, timeout=300):
    ...
    # P1-J/H09: pinned connection (owner's intentional local door keeps its
    # key - policy "any" still vettes internal-RESERVED). The P1-A docstring's
    # "documented imperfection" line is hereby retired.
    _pol = "any" if _check is None else "public"
    _opener = _p1j_build_opener(_P1cRedirectGuard(_check), _pol)
...
def _f17_post_opener():
    ...
    if _P1G_F17_OPENER is None:
        _P1G_F17_OPENER = _p1j_build_opener(
            _P1cRedirectGuard(_f17_guard_base), "public")  # P1-J/H09
...
def _family_source(username):
    ...
    if u is None:
        return username, False
    if _is_share_principal(username):
        # S06 (0.6q): a share rides the SHARER's provider + keys (K80:
        # "API calls are the person sharing API and model as well as tiers."
        # Keys are NEVER copied into the share namespace - custody stays
        # in the sharer's write-only rows (same pattern as family above).
        # Dead share -> self-reliance (which has no keys -> clean 503).
        _sh = share_get_by_principal(username)
        if _sh is not None and _share_alive(_sh):
            return _sh["owner"], True
        return username, False
    try:
...
def agent_loop(messages: list, model_cfg: dict, send_event, cancel=None, username=None, allow_tools=True, tool_names=frozenset(), conv_id=None, files_sink=None):
    ...
    for iteration in range(MAX_TOOL_ITERATIONS):
        ...
        if allow_tools and iteration < MAX_TOOL_ITERATIONS - 1:
            ...
            if tool_names is not None and SHARE_ASK_NAME in tool_names:
                # S06r: the ask pseudo-tool rides OUTSIDE the TOOLS tier
                # universe on purpose - it grants nothing and belongs to no
...
<div class="card" id="tier0Card" style="display:none">
  ...
  <div class="row" style="margin:0 0 8px"><label>Scope</label>
    <select id="t0Scope" onchange="loadTier0()">
      <option value="global">Instance — binds everyone except you</option>
      <option value="personal">Personal — binds only you</option>
    </select>
    <button class="btn" id="t0CreatePersonal" style="display:none;padding:2px 10px" onclick="createPersonalTier0()">start from instance copy</button></div>
  <p id="t0ScopeHint" style="font-size:12px;opacity:.65;margin:0 0 10px"></p>
...
// ── F29 R4: Connection & TLS card. Owner-only endpoint; its own fetch pair,
// deliberately NOT part of saveSettings (transport config is machine-wide).
async function tlsPost(b) {
  const r = await fetch('api/tls', {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'}, body: JSON.stringify(b)});
  let j = {}; try { j = await r.json(); } catch (e) {}
  return {st: r.status, j: j};
}
function tlsMsg(id, good, t) { const el = document.getElementById(id); if (el) { el.textContent = t; el.style.color = good === true ? 'var(--ok, #2e7d32)' : (good === false ? '#b3261e' : 'inherit'); } }
async function loadTlsCard() {
  const el = document.getElementById('tlsBody'); if (!el) return;
  let s = null;
  try { const r = await fetch('api/tls', {credentials:'same-origin'});
    if (!r.ok) { if (r.status === 401 || r.status === 403) { const c = document.getElementById('tlsCard'); if (c) c.style.display = 'none'; } else { el.textContent = 'TLS status unavailable (HTTP ' + r.status + ').'; } return; }
    s = await r.json();
  } catch (e) { el.textContent = 'TLS status unavailable.'; return; }
  const esc = (x) => String(x == null ? '' : x).replace(/&/g, String.fromCharCode(38) + 'amp;').replace(/</g, String.fromCharCode(60) + 'lt;').replace(/>/g, String.fromCharCode(62) + 'gt;').replace(/"/g, String.fromCharCode(38) + 'quot;');
  const modes = [['proxy', 'Proxy - TLS handled by Caddy/nginx (default)'], ['local', 'Local CA - daemon serves HTTPS itself'], ['external', 'Existing certificate - paths you provide']];
  el.innerHTML =
    '<div class="row"><label>Mode</label><select id="tlsMode">' + modes.map(function (m) { return '<option value="' + m[0] + '"' + (s.mode === m[0] ? ' selected' : '') + '>' + m[1] + '</option>'; }).join('') + '</select></div>' +
    '<div class="row"><button class="btn" onclick="tlsSaveMode()">Apply mode</button> <span id="tlsModeMsg" style="font-size:12px"></span></div>' +
    '<div class="hint" style="margin:8px 0">Right now this socket is ' + (s.tls_served ? '<b>HTTPS</b>' : 'plain HTTP (fine behind a TLS proxy)') + (s.env_lock ? ' &middot; <b>MARA_TLS_MODE env lock is active (' + esc(s.env_lock) + ') - the environment wins over this page until it is removed</b>' : '') + (s.restart_required ? ' &middot; <b>restart required</b> to apply the saved mode' : '') + '.</div>' +
    '<div style="border-top:1px solid var(--border);margin:12px 0"></div>' +
    '<div class="row"><label>Extra names</label><input id="tlsSan" placeholder="hostnames / IPs your devices will use, e.g. myhome.local or a LAN address" style="max-width:420px"></div>' +
    '<div class="row"><button class="btn" onclick="tlsGenerate()">' + (s.ca_exists ? 'Reissue host cert (CA kept)' : 'Generate local CA + host cert') + '</button> <a class="btn" style="text-decoration:none' + (s.ca_exists ? '' : ';opacity:.5;pointer-events:none') + '" href="api/tls/ca.crt">Download CA certificate</a> <span id="tlsGenMsg" style="font-size:12px"></span></div>' +
    '<div class="hint" style="margin:8px 0">Install the CA once per device (Windows: certutil -addstore or double-click &rarr; install to Trusted Root; macOS/iOS: open the file and install the profile; Android & Linux: per-distro CA install). Then use <b>https://</b> with the names above.</div>' +
    '<div style="border-top:1px solid var(--border);margin:12px 0"></div>' +
    '<div class="row"><label>Cert path</label><input id="tlsCert" value="' + esc(s.cert_path) + '" style="max-width:420px" autocomplete="off" placeholder="/etc/ssl/... your fullchain.pem"></div>' +
    '<div class="row"><label>Key path</label><input id="tlsKey" style="max-width:420px" autocomplete="off" placeholder="' + (s.key_path_present ? 'stored (write-only) - retype to replace' : '/etc/ssl/... your privkey.pem') + '"></div>' +
    '<div class="row"><button class="btn" onclick="tlsSaveCert()">Validate + install existing cert</button> <button class="btn" onclick="tlsRestart()">Restart daemon</button> <span id="tlsCertMsg" style="font-size:12px"></span></div>' +
    (s.cert ? '<div class="hint" style="margin-top:8px">Cert on disk: <code>' + esc(s.cert) + '</code> &middot; ' + (s.cert_valid ? 'valid, not expired' : '<b>' + esc(s.cert_error || 'not valid') + '</b>') + '</div>' : '');
}
async function tlsSaveMode() { const m = document.getElementById('tlsMode').value; const r = await tlsPost({action: 'mode', mode: m}); tlsMsg('tlsModeMsg', r.st === 200, r.st === 200 ? 'saved - restart the daemon to apply' : 'failed: ' + ((r.j && r.j.error) || r.st)); loadTlsCard(); }
async function tlsGenerate() { tlsMsg('tlsGenMsg', null, 'generating (RSA keygen can take a few seconds)...'); const r = await tlsPost({action: 'generate', san: document.getElementById('tlsSan').value}); tlsMsg('tlsGenMsg', r.st === 200, r.st === 200 ? ('issued for: ' + ((r.j.sans || []).join(', ')) + (r.j.ca_created ? ' - new CA created' : ' - existing CA kept')) : ('failed: ' + ((r.j && r.j.error) || r.st))); loadTlsCard(); }
async function tlsSaveCert() { tlsMsg('tlsCertMsg', null, 'validating pair...'); const r = await tlsPost({action: 'save-cert', cert: document.getElementById('tlsCert').value, key: document.getElementById('tlsKey').value}); tlsMsg('tlsCertMsg', r.st === 200, r.st === 200 ? 'pair valid, saved, mode=external - restart to serve it' : ('rejected: ' + ((r.j && r.j.error) || r.st))); loadTlsCard(); }
async function tlsRestart() { if (!confirm('Restart the daemon now? Chats pause for a second; unsaved edits elsewhere are lost.')) return; tlsMsg('tlsCertMsg', null, 'restarting...'); try { await tlsPost({action: 'restart'}); } catch (e) {} setTimeout(loadTlsCard, 2600); }
...
// F29/R2 (Q3): scope selector - 'global' = instance constitution (binds all but
// the owner), 'personal' = the owner's own tier0 (binds only the owner).
window._t0Scope = 'global';
window._t0Personal = false;
window._t0GlobalText = '';
function _t0scopeVal() { const s = document.getElementById('t0Scope'); return (s && s.value === 'personal') ? 'personal' : 'global'; }
async function loadTier0(scope) {
  const card = document.getElementById('tier0Card');
  if (scope) window._t0Scope = scope;
  try {
    const r = await fetch('api/tier0?scope=' + window._t0Scope);
    if (!r.ok) { card.style.display = 'none'; return; }
    const d = await r.json();
    card.style.display = '';
    window._t0Personal = !!d.personal_exists;
    if (window._t0Scope === 'global') window._t0GlobalText = d.text || '';
    document.getElementById('t0Text').value = d.text || '';
    const sel = document.getElementById('t0Scope'); if (sel) sel.value = window._t0Scope;
    document.getElementById('t0CreatePersonal').style.display =
...
function createPersonalTier0() {
  // F29/R2: seed the personal file from the instance copy (snapshot, like tier1 pristine semantics).
  document.getElementById('t0Scope').value = 'personal';
  loadTier0('personal').then(() => {
    document.getElementById('t0Text').value = window._t0GlobalText;
    updT0Count();
    setStatus('t0Status', 'warn', 'instance copy loaded into the Personal editor - Save to make it real');
  });
}
function updT0Count() {
  const n = document.getElementById('t0Text').value.length;
  const who = _t0scopeVal() === 'personal' ? 'ONLY your prompts' : 'EVERY prompt, for EVERY principal';
  document.getElementById('t0Count').textContent = n
    ? (n + ' chars - this text rides ' + who + ', to EVERY provider')
    : 'unwelded - nothing is prepended';
}
document.getElementById('t0Text').addEventListener('input', updT0Count);
async function saveTier0() {
  const sc = _t0scopeVal();
  try {
    const r = await fetch('api/tier0', { method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ action: 'save', scope: sc, text: document.getElementById('t0Text').value }) });
    const d = r.ok ? await r.json() : (await r.json().catch(() => ({})));
    setStatus('t0Status', r.ok ? 'ok' : 'warn', r.ok ? ('Saved (' + sc + ') - ' + d.chars + ' chars; ' + (sc === 'personal' ? 'only your next messages carry it' : "every principal's next message carries it")) : (d.error || ('save failed (HTTP ' + r.status + ')')));
    if (r.ok) loadTier0();
...
async function resetTier0() {
  try {
    const r = await fetch('api/tier0', { method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ action: 'reset', scope: _t0scopeVal() }) });
    const d = r.ok ? await r.json() : (await r.json().catch(() => ({})));
...
async function loadUserRoles() {
  ...
  try {
    ...
    list.forEach(p => {
      ...
      // C02 (K80 law 2026-09-25): staff moderation. This list is already
      // 403-fenced to active staff server-side, and every action below is
      // re-checked server-side; the buttons are cosmetic affordances only.
      ...
      kb.className = 'btn'; kb.style.marginLeft = '8px';
      kb.textContent = 'Disable ALL shares';
      kb.title = 'Kill switch: disable every share this user owns, kick guests, stop in-flight runs. Only the sharer can re-enable.';
      kb.onclick = () => killShares(p.username);
      el.appendChild(kb);
      if (p.role !== 'owner') {
        const bb = document.createElement('button');
        bb.className = 'btn'; bb.style.marginLeft = '4px';
        if (p.status === 'suspended') {
          bb.textContent = 'Restore account';
          bb.onclick = () => restoreUser(p.id, p.username);
        } else {
          bb.textContent = 'Break account';
          bb.title = 'Suspend the account, kill all sessions, disable all their shares. Owner-only restore.';
          bb.onclick = () => breakUser(p.id, p.username);
        }
        el.appendChild(bb);
      }
      div.appendChild(el);
...
async function killShares(name) {
  if (!confirm('Disable ALL shares of "' + name + '"? Their guests are cut off immediately and in-flight generations stop. Only ' + name + ' can re-enable them later.')) return;
  try {
    const r = await fetch('api/shares', { method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ action: 'disable_all', username: name }) });
    const d = await r.json().catch(() => ({}));
    setStatus('userRoleStatus', 'warn', r.ok
      ? (name + ': ' + (d.disabled || 0) + ' share(s) disabled, ' + (d.cancelled_runs || 0) + ' in-flight run(s) cancelled')
      : (d.error || ('kill switch failed (HTTP ' + r.status + ')')));
  } catch (e) { setStatus('userRoleStatus', 'warn', 'kill switch failed (network)'); }
}
async function breakUser(id, name) {
  if (!confirm('BREAK the account "' + name + '"? They are suspended, logged out of every device, and every share of theirs is disabled. Only the owner can restore them.')) return;
  try {
    const r = await fetch('api/users/' + id + '/break', { method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ action: 'break' }) });
    const d = await r.json().catch(() => ({}));
    setStatus('userRoleStatus', 'warn', r.ok
      ? (name + ': account broken (' + (d.shares_disabled || 0) + ' share(s) halted)')
      : (d.error || ('break failed (HTTP ' + r.status + ')')));
    if (r.ok) loadUserRoles();
  } catch (e) { setStatus('userRoleStatus', 'warn', 'break failed (network)'); }
}
async function restoreUser(id, name) {
  if (!confirm('Restore the suspended account "' + name + '"?')) return;
  try {
    const r = await fetch('api/users/' + id + '/break', { method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ action: 'restore' }) });
    const d = await r.json().catch(() => ({}));
    setStatus('userRoleStatus', 'warn', r.ok ? (name + ': restored - they can sign in again')
...
# H16 (audit 2026-09-24, K80 "no half-backed fixes"): stock
# ThreadingHTTPServer is thread-per-connection with NO admission control -
# any client that can reach the port can park as many workers as it likes,
# each held for the full socket timeout. The semaphore below is the budget
# the server never had: over budget gets an immediate 503 + close, never a
# queued thread. Env MARA_MAX_WORKERS (default 64) is the whole knob; the
...
class MaraHandler(BaseHTTPRequestHandler):
    timeout = 300  # P1-B/H16: per-connection socket guard. 300s sits exactly
                   ...
                   # needs more, and a hung socket no longer squanders a
                   # worker for fifteen minutes. Admission is the semaphore.
    ...
    def _cookie_secure(self):
        # R4: Secure belongs to what the browser sees: our own TLS socket, or
        # https at the front (X-Forwarded-Proto). Plain http on a LAN is a real
        # community setup - constant-Secure cookies are silently dropped there
        # and login dies without one readable console error.
        if TLS_SERVED:
            return True
        try:
    ...
    def do_GET(self):
        ...
        if path == "/shares":
            if not self._auth_user():
                ...
                return
            self._html(200, WEB_UI_SHARES)
            return
        if path == "/share" or path.startswith("/share/"):
            _share_route_get(self, path)
            return
        if path == "/setup":
        ...
        if path == "" or path == "/":
        ...
        elif path == "/api/shares":  # C02: my shares, self-only
            _shares_api_get(self)
        elif path == "/api/share-approvals":  # C02: my shares\' inbox, self-only
            _share_approvals_api_get(self)
        elif path == "/api/share-memories":  # C02: my shares\' memories, self-only
            _share_memories_api_get(self)
        elif path == "/api/me":
            u = self._auth_user()
            if u:
            ...
            # L35 (audit round 8, 0.6u): the detail blob carries the OWNER's
            # model/provider/key state, build sha and pid - owner role only.
            # Signed-in non-owners (users, share guests) get liveness only.
            _hu = self._auth_user()
            if not (_hu and _hu["status"] == "active" and _hu["role"] == "owner"):
            ...
            with sqlite3.connect(DB_PATH) as db:
                ...
                rows = db.execute("SELECT id, title, created_at, updated_at, model_override, guest_id FROM conversations WHERE user_id=? ORDER BY updated_at DESC LIMIT 100", (u["username"],)).fetchall()
                # S06 (0.6s): the history box. C03 (0.6v) replaced the old
                # "most recent one" slice with REAL per-guest isolation:
                # allow_history=0 (default) => a guest lists ONLY threads
                # minted during its own visit; legacy NULL-stamp threads stay
                # sealed until the sharer opens the group room. history=1 =>
                # full list, group-room consent (K80 ruling 2026-09-25).
                # Threads stay in the DB either way (merge-back review canon).
                # Fail closed: a share session with no mintable guest id lists
                # nothing. Ordinary principals are untouched, byte-for-byte
                # (guest_id now rides the row dicts; clients read named fields).
                if _is_share_principal(u["username"]) and not _share_history_allowed(u["username"]):
                    _g03 = self._sess_guest_id()
                    rows = [r for r in rows if r["guest_id"] == _g03] if _g03 is not None else []
            ...
            # C02 (0.6u): suspended rows stay LISTED (with their status) so
            # the owner can see and RESTORE broken accounts; the role endpoint
            # already refuses to edit non-active rows.
            u = self._auth_user()
            if not u or u["role"] not in ("admin", "owner") or u["status"] != "active":
                self._json(403, {"error": "admin only"})
                return
            with _reg_db() as db:
                rows = db.execute("SELECT id, username, display_name, agent_name, slug, role, status, created_at FROM users WHERE status IN ('active','suspended') ORDER BY created_at").fetchall()
            if u["role"] != "owner":
                rows = [r for r in rows if r["role"] in ("user", "owner") or r["id"] == u["id"]]
            # S06: share principals are infrastructure, not residents -
            # they never appear in any human user list.
            rows = [r for r in rows if r["role"] != "guest"]
            self._json(200, [dict(r) for r in rows])
        elif path == "/api/tls":
            # F29 R4: connection/TLS status. Owner-only (Q3 family: transport
            # config and cert paths are the owner's machine-level business).
            u = self._auth_user()
            if not u or u["status"] != "active":
            ...
            if u["role"] != "owner":
                self._json(403, {"error": "owner only"})
                return
            self._json(200, _tls_status())
        elif path == "/api/tls/ca.crt":
            # F29 R4: the local CA public half, for devices that must trust it.
            # Any active account may download: a CA certificate is a public
            # artifact (the private half never leaves state/tls/). Anon refused.
            u = self._auth_user()
            if not u or u["status"] != "active":
                self._json(401, {"error": "authentication required"})
                return
            _r4p = str(TLS_DIR / "ca.crt")
            if not os.path.exists(_r4p):
                self._json(404, {"error": "no local CA yet - generate one in Settings > Connection & TLS"})
                return
            with open(_r4p, "rb") as _r4f:
                _r4b = _r4f.read()
            self.send_response(200)
            self.send_header("Content-Type", "application/x-x509-ca-cert")
            self.send_header("Content-Length", str(len(_r4b)))
            self.send_header("Content-Disposition", 'attachment; filename="cairn-local-ca.crt"')  # static name (P1-E/J class: name is never data-derived here)
            self.end_headers()
            self.wfile.write(_r4b)
        elif path == "/api/setup/tiers":
            # F29/R3: tier walkthrough catalog for the wizard's Personality step (Q5).
            # READ-ONLY: writes go through the EXISTING owner endpoints, so this adds
            # no new write surface. Owner-only because previews carry Tier-0/1 text,
            # which is owner-invisible-to-others by Q1. Stays available after setup
            # (harmless: owner-scoped reads of the owner's own files).
            u = self._auth_user()
            if not u or u["status"] != "active":
                self._json(401, {"error": "authentication required"})
                return
            ...
            try:
                if SYSTEM_PROMPT_PATH.exists():
                    _t1 = SYSTEM_PROMPT_PATH.read_text()
            except Exception:
                _t1 = ""
            # same substitution the wire projection makes (P3.3 S3n): preview must
            # show the literal agent name, never the placeholder.
            _an = _registry_agent_name(u["username"]) or u["username"]
            if "Your name is your agent_name." in _t1:
            ...
            # F26: constitution reader. OWNER ONLY. F29/R2 (Q3): optional
            # ?scope=personal reads the owner's OWN tier0; default scope
            # reads the instance (global) constitution. Authenticated
            # non-owner = 403 either way (everyone may know Tier 0 exists,
            # only the owner may ever read its text).
            u = self._auth_user()
            if not u or u["status"] != "active":
                self._json(401, {"error": "authentication required"})
                return
            if u["role"] != "owner":
                self._json(403, {"error": "owner only"})
                return
            _scope = dict(_nc_up.parse_qsl(_nc_up.urlsplit(self.path).query)).get("scope", "global")  # house query pattern (no self._query exists)
            _tpath = _tier0_write_target(u["username"], _scope) if _scope == "personal" else TIER0_PATH
            if _tpath is None:
                self._json(400, {"error": "bad scope"})
                return
            _t0 = ""
            try:
                if _tpath.exists():
                    _t0 = _tpath.read_text()
            except Exception:
                _t0 = ""
            _ppristine = _tpath.with_name(_tpath.name + ".pristine")
            self._json(200, {"text": _t0, "exists": bool(_t0), "chars": len(_t0),
                             "scope": "personal" if _tpath != TIER0_PATH else "global",
                             "personal_exists": (IDENTITY / (TIER0_PERSONAL_FMT % DAEMON_OWNER)).exists(),
                             "pristine_exists": _ppristine.exists()})
    ...
    def _auth_user(self):
        ...
        if not uname:
            return None
        _gu = registry_get_user(uname)
        # S06 (0.6q): the GUEST FENCE - deny by default. A role=guest
        # session reads as anonymous everywhere except the share
        # allowlist; every scoped route below keeps its own owner checks.
        if _gu is not None and (_gu["role"] or "") == "guest":
            if not _share_path_allowed(_p1i_clean_path(self.path).split("?")[0]):
                return None
        return _gu
    ...
    def _sess_guest_id(self):
        # C03 (0.6v): THIS request's guest identity, read off its own session
        # row. Legacy share sessions (pre-C03, guest_id NULL) self-heal: mint
        # once, ride the row from then on. Cached per request. The cookie
        # regex is the exact anchored shape from _auth_user (P1-C/F10).
        ...
        if m:
            ...
            with _reg_db() as db:
                row = db.execute("SELECT guest_id FROM sessions WHERE token=?", (htok,)).fetchone()
            if row is not None:
                gid = row[0]
                if gid is None:
                    gid = os.urandom(8).hex()
                    with _reg_db() as db:
                        db.execute("UPDATE sessions SET guest_id=? WHERE token=?", (gid, htok))
        self._c03_gid = gid
        return gid
    def _guest_can_conv(self, username, conv_guest):
        # C03 (0.6v) consent model (K80 ruling 2026-09-25): allow_history ON =
        # GROUP ROOM, every guest of the share sees every thread - that is
        # what the box means. OFF (default) = only threads minted during THIS
        # visit. Legacy threads (guest_id NULL, born pre-C03) belong to no
        # visit: they surface ONLY in the group room, never guessed at.
        # Fail closed: no guest id, no threads. Ordinary principals: True.
        if not _is_share_principal(username):
            return True
        if _share_history_allowed(username):
            return True
        gid = self._sess_guest_id()
        return gid is not None and conv_guest == gid
    def _conv_owner(self, conv_id, username):
        # P3.3 S2: True only if the conversation exists and belongs to username.
        # C03 (0.6v): inside a share principal, per-guest isolation applies too.
        with sqlite3.connect(DB_PATH) as db:
            row = db.execute("SELECT user_id, guest_id FROM conversations WHERE id=?", (conv_id,)).fetchone()
    ...
    def _door_bounce(self):
        ...
        # sit ABOVE this check was the bypass - a direct (loopback or
        # proxy-misconfigured) request simply carried no X-Mara-Slug and walked
        # past the principal fence with any resident's cookie. Local truth now
        # governs EVERY private route; the header, when present, is only ever
        # corroboration (a disagreeing header is a lie about which door this
        # is and bounces to that door's own login, which cannot serve here).
        ...
        if _trusted:
            ...
            if not (u and u["slug"] == _trusted):
                ...
                return True
            return False
        # No local instance truth (single-owner shape - today's CAIRN): the
        # header, when present, stays the only available check. Absence is the
        # documented legacy pass for that shape ONLY, and only while no
        # instance.conf exists. Put an instance.conf down and absence is
...
# ---- S4f11: creds vault. Entered, encrypted, never returned. ----
    ...
    def do_POST(self):
        ...
        if path == "/api/share/login":  # S06 guest door (rate-limited inside)
            _share_login(self)
            return
        if path == "/api/shares":  # S06 share management (actions inside)
            _shares_api_post(self)
            return
        if path == "/api/shares/presence":  # S06r presence beacon (sharer side)
            _share_presence_post(self)
            return
        if path == "/api/share-approvals":  # S06r inbox decisions
            _share_approvals_api_post(self)
            return
        if path == "/api/share-memories":  # S06s memory review actions
            _share_memories_api_post(self)
            return
        ...
        if path == "/api/chat":
            ...
            if not u:
                ...
                return
            # 0.6t N4 (audit round 7, defense-in-depth): registry password login
            # NEVER hands out sessions to share principals (or any guest-role
            # row). The share nozzle is the ONLY door a clone authenticates
            # through - a slug/role fence here keeps it that way even if a
            # future migration changes what authenticate() may return.
            if (u["role"] or "") == "guest" or str(u["slug"] or "").startswith("share-"):
                if _logs_user_exists(u["username"]):
                    log_event(u["username"], "auth.login.failure", reason="share_principal")
                self._json(403, {"error": "share principals cannot sign in here - use the share link"})
                return
            if u["status"] != "active":
            ...
            self._json(200, {"ok": True}, cookie=_clear_cookie(self._cookie_secure()))
            ...
            # F29 R4: owner-only transport control. Actions: mode | generate |
            # save-cert | restart. ACME is deliberately NOT one of them (K80
            # hard rule): this daemon never talks to certificate authorities.
            u = self._auth_user()
            if not u or u["status"] != "active":
                self._json(401, {"error": "authentication required"})
                return
            ...
            if action == "mode":
                ...
                if m not in TLS_MODES:
                    self._json(400, {"error": "mode must be proxy, local, or external"})
                    return
                set_setting("tls_mode", m, DAEMON_OWNER)
                self._json(200, {"ok": True, "mode": m, "restart_required": True,
                                 "env_lock": os.environ.get("MARA_TLS_MODE", "").strip() or None})
            elif action == "generate":
                try:
                    res = _tls_generate(str(body.get("san") or ""))
                except RuntimeError as e:
                    self._json(400, {"error": str(e)})
                    return
                except Exception as e:
                    self._json(500, {"error": "generation failed: " + type(e).__name__})
                    return
                self._json(200, {"ok": True, "ca_created": res["ca_created"], "sans": res["sans"],
                                 "note": ("local CA created (kept for future reissues)" if res["ca_created"]
                                          else "existing local CA reused - the CA never rotates silently")})
            elif action == "save-cert":
                cert = str(body.get("cert") or "").strip()
                key = str(body.get("key") or "").strip()
                _oldc, _oldk = _tls_ext_paths()
                if not cert and not key:
                    self._json(400, {"error": "nothing to save - both fields are empty"})
                    return
                # key field is write-only (never echoed back): leaving it empty
                # keeps the stored pair if the cert path is unchanged.
                if not key and cert and cert == _oldc and _oldk:
                    key = _oldk
                if not cert or not key:
                    self._json(400, {"error": "both a certificate and a key path are required (the key field is write-only; retype it with the cert if unsure)"})
                    return
                # Validate the PAIR before storing anything: never accept a path
                # that would boot the daemon into a dead TLS listener.
                err = _tls_validate_pair(cert, key)
                if err:
                    self._json(400, {"error": err})
                    return
                set_setting("tls_cert_path", cert, DAEMON_OWNER)
                set_setting("tls_key_path", key, DAEMON_OWNER)
                if _tls_mode() != "external":
                    set_setting("tls_mode", "external", DAEMON_OWNER)
                self._json(200, {"ok": True, "mode": "external", "restart_required": True,
                                 "note": "pair validated and saved; mode set to external - restart to serve it"})
            elif action == "restart":
                # Same re-exec trick the F20 updater uses: same PID, systemd
                # keeps the unit; sessions live in the DB so browsers survive.
                ...
                try:
                    ...
                    pass
                try:
                    os.execv(sys.executable, [sys.executable, os.path.abspath(__file__)] + list(sys.argv[1:]))
                except Exception:
                    pass  # client already has its 200; the worst case is "restart manually"
                return
            else:
                self._json(400, {"error": "action must be mode, generate, save-cert, or restart"})
        elif path == "/api/tier0":
            # F26: the constitution. OWNER ONLY. F29/R2 (Q3 ruling): body
            # may carry scope="personal" to write the owner's OWN tier0
            # (binds only the owner); default scope writes the instance
            # constitution. Both reload_identity() through _tier_commit.
            u = self._auth_user()
            if not u or u["status"] != "active":
                self._json(401, {"error": "authentication required"})
                return
            ...
            if _scope not in ("global", "personal"):
                self._json(400, {"error": "scope must be global or personal"})
                return
            _tpath = _tier0_write_target(u["username"], _scope)
            if _tpath is None:
                self._json(400, {"error": "cannot resolve tier0 write target"})
                return
            _ppristine = _tpath.with_name(_tpath.name + ".pristine")
            action = body.get("action")
            if action == "save":
                ...
                if len(text.encode("utf-8")) > TIER0_MAX_BYTES:
                    ...
                    return
                try:
                    _tier_commit(_tpath, text, "tier0 save" + ("" if _tpath == TIER0_PATH else " (personal)"), u["username"], pristine_src=_ppristine)
                except Exception:
                    self._json(500, {"error": "tier0 save failed (see daemon log)"})
                    return
                self._json(200, {"ok": True, "chars": len(text), "scope": _scope})
            elif action == "reset":
                if not _ppristine.exists():
                    self._json(404, {"error": "no pristine tier0 for this scope"})
                    return
                try:
                    _tier_commit(_tpath, _ppristine.read_text(), "tier0 reset" + ("" if _tpath == TIER0_PATH else " (personal)"), u["username"])
                except Exception:
            ...
            # ACCOUNT BREAK (K80 green light 2026-09-25 19:21): one motion for
            # "this persons account is the problem" - suspend status, kill all
            # sessions, kill-switch every share of theirs (immediate halt).
            # Matrix mirrors the role endpoint: owner breaks any active
            # non-owner; admin breaks user rows; the owner row is NOT
            # breakable through the UI (no self-lockout; recovery is operator
            # CLI). Restore is owner-only. "suspended" is distinct from
            # "deprovisioned" (which implies wiped state); the login path
            # already refuses every non-active status with 403 (verified
            # 0.6t: status gate fires after the guest-principal fence).
            u = self._auth_user()
            if not u or u["role"] not in ("admin", "owner") or u["status"] != "active":
                self._json(403, {"error": "admin only"})
                return
            m = re.fullmatch(r"/api/users/([a-f0-9-]{8,64})/break", path)
            if not m:
                self._json(404, {"error": "not found"})
                return
            target = registry_get_by_id(m.group(1))
            if not target or (target["role"] or "") == "guest":
                self._json(404, {"error": "not found"})
                return
            try:
                body = self._read_body()
            ...
            if not isinstance(body, dict):
                self._json(400, {"error": "JSON body must be an object"})
                return
            action = body.get("action") or "break"
            if action == "restore":
                if u["role"] != "owner":
                    self._json(403, {"error": "owner only"})
                    return
                if target["status"] != "suspended":
                    self._json(400, {"error": "user is not suspended"})
                    return
                registry_set_status(target["id"], "active")
                log_event(u["username"], "user.unsuspend", target=target["username"])
                self._json(200, {"ok": True, "status": "active"})
                return
            if action != "break":
                self._json(400, {"error": "action must be break or restore"})
                return
            if (target["role"] or "") == "owner":
                self._json(400, {"error": "owner account is not breakable here"})
                return
            if target["role"] == "admin" and u["role"] != "owner":
                self._json(403, {"error": "owner required to break an admin"})
                return
            if target["status"] != "active":
                self._json(400, {"error": "user is not active"})
                return
            if target["id"] == u["id"]:
                # Self-break is nobody's lever: an admin who suspends their own
                # account just opens a recovery ticket for the owner. Not in
                # K80's break matrix; refused explicitly.
                self._json(400, {"error": "cannot break your own account"})
                return
            registry_set_status(target["id"], "suspended")
            sessions_delete_all(target["username"])
            with _reg_db() as db:
                rows = db.execute("SELECT * FROM shares WHERE owner=?", (target["username"],)).fetchall()
            n = 0
            principals = []
            for r in rows:
                shx = dict(r)
            ...
            self._json(200, {"ok": True, "status": "suspended",
                             "shares_disabled": n, "cancelled_runs": cancelled})
            ...
            if target["role"] == "guest":
                # S06: guest rows are managed by the shares surface only.
                self._json(404, {"error": "not found"})
                return
    ...
    def _handle_conv_delete(self, cid):
        ...
        with sqlite3.connect(DB_PATH) as db:
            db.row_factory = sqlite3.Row
            row = db.execute(
                "SELECT id, title, user_id, guest_id FROM conversations WHERE id=?",
                (cid,)).fetchone()
            # C03 (0.6v): guests delete only their own visit's threads.
            if not row or row["user_id"] != u["username"] or not self._guest_can_conv(u["username"], row["guest_id"]):
                self._json(404, {"error": "not found"})
                return
            if cid in _STREAMS:
    ...
    def _handle_chat(self):
        ...
        if not u:
            return
        uname = u["username"]
        # S06 (0.6q): share principals - revocation and the guest nozzle
        # re-checked at request time; True means a response was sent.
        if _is_share_principal(uname) and _share_gate(self, u):
            return
        ...
        if not message and not att_ids:
            ...
            return
        # C03 (0.6v): stamp every conversation with the guest that minted it;
        # enforce the stamp while the share's history box is off. Group-room
        # shares (history on) stamp for the record but never enforce - guests
        # join each other's threads BY CONSENT (K80 ruling 2026-09-25).
        _g03 = None
        _g03_enf = False
        if _is_share_principal(uname):
            _g03 = self._sess_guest_id()
            _g03_enf = not _share_history_allowed(uname)
        verdict, conv_id = _p1i_claim_conv(cid_in, uname, guest_id=_g03, enforce_guest=_g03_enf)
        if verdict == "mint":
            conv_id = str(uuid.uuid4())
            verdict, conv_id = _p1i_claim_conv(conv_id, uname, guest_id=_g03, enforce_guest=_g03_enf)
        if verdict == "bad":
            self._json(400, {"error": "invalid conversation_id"})
            return
        if verdict == "denied":
            # P3.3 S2 posture kept: foreign conversations are invisible.
            # C03 (0.6v): a guest whose session rotated (disable->enable,
            # expiry, logout-all) carries a stale localStorage conversation
            # that now belongs to a sealed earlier visit. The kind answer is
            # a brand-new thread, not a dead page - the old thread stays
            # exactly as sealed as it is. Ordinary users keep the 404.
            if _g03_enf and _g03 is not None:
                verdict, conv_id = _p1i_claim_conv(str(uuid.uuid4()), uname,
                                                   guest_id=_g03, enforce_guest=True)
                if verdict != "ok":
                    self._json(404, {"error": "not found"})
                    return
            else:
        ...
        def send_event(event_type, data):
            # H12: the recorder appends moved INSIDE _p1i_dispatch's critical
            # section - record and fan-out are now one atomic act. Socket
            # writes stay OUTSIDE the lock (P1-I/S05 invariant: a stalled
    ...
    def _handle_stream_reattach(self, conv_id, username=None):
        ...
        if username is not None and not self._conv_owner(conv_id, username):
            ...
            self._json(404, {"error": "not found"})
            return
        # H12 (round-8 audit, 2026-09-25): lookup, capacity check, listener
        # registration and the replay snapshot are ONE critical section - the
        # same lock _p1i_dispatch holds while it records and fans out. The
        # old shape replayed, registered, then replayed AGAIN (whole prefix
        # twice on the wire), answered "too many viewers" with a _json AFTER
        # SSE headers (a malformed frame, not a refusal), and drained with
        # get_nowait() (queue.Empty threw on the first quiet millisecond of
        # an alive generation). Register-then-snapshot under the dispatcher's
        # own lock means every event is in the snapshot OR in the queue:
        # never both, never neither. No seq bookkeeping is needed anymore.
        with _SSE_LOCK:
            ...
            if rec is None:
                ...
                return
            if len(rec["listeners"]) >= 4:
                # refusal BEFORE any SSE byte (S05's cap, honest delivery)
                self._json(503, {"error": "too many viewers on this conversation"})
                return
            q = Queue(maxsize=256)
            entry = {"q": q, "dead": False}
            rec["listeners"].append(entry)
        ...
        try:
            ...
            if snap["full"]:
                ...
                # sentinel. Give it two heartbeats, then leave cleanly.
                try:
                    q.get(timeout=2)
                except Exception:
                    pass
            ...
            while True:
                try:
                    ...
                    if entry["dead"]:
                        break   # dispatcher detached this viewport; the
                                # frontend re-attaches - do NOT tail a queue
                                # that can no longer receive the sentinel
                    emit("status", {"phase": "working"})
                    continue
                if evt is None:
                    break       # generation finished; sentinel from close
                emit(evt[0], evt[1])
        except (BrokenPipeError, ConnectionResetError, OSError):
            pass  # viewport vanished; generation continues - that IS the point
        finally:
    ...
    def _handle_export(self, path, username):
        ...
        with sqlite3.connect(DB_PATH) as db:
            db.row_factory = sqlite3.Row
            c = db.execute("SELECT id, title, created_at, updated_at, user_id, guest_id FROM conversations WHERE id=?", (conv_id,)).fetchone()
            # C03 (0.6v): defense-in-depth - role=guest cannot reach export
...
_F20_OPENER = _p1j_build_opener(_F20NoRedirect(), "lan_ok")  # P1-J/H09
                                   # lan_ok = the floor _f20_guard already
                                   # enforces (reserved refused; posture is
                                   # the guard's job, the dial is ours)
...
def _f20_snapshot(stamp):
    ...
    # entries under backups/ are NEVER touched. Prune failure never fails a
    # snapshot - the write you just made is the safety net, not this.
    try:
        _keep = sorted([p for p in _F20_BACKUPS.glob("pre-update-*") if p.is_dir()])
        for _old in _keep[:-5]:
            try:
                _f20_shutil.rmtree(str(_old))
                _f20_clog("AA prune: removed old snapshot " + _old.name)
            except Exception as e:
...
# ─── F29 R4: Connection & TLS (K80 ruling 2026-09-24 10:12 CDT) ──────────────
# Machine-wide transport switch, three modes (it is transport, not a per-user
# preference; stored on the owner settings row via derived DAEMON_OWNER and
# deliberately kept OFF the /api/settings key whitelist - dedicated /api/tls
# endpoints own it, owner-only):
#   proxy    (default) plain HTTP; TLS belongs to whatever fronts us (Caddy/nginx).
#   local    the daemon serves TLS itself with a locally-generated CA + host cert
#            (openssl subprocess; files under state/tls/; keys 0600; CA never
#            rotates under your fingers - reissue reuses the CA, UI says so).
#   external user-supplied cert+key paths; the pair is validated BEFORE saving
#            (fail-closed) and re-checked at boot.
# HARD RULE (K80): no ACME client is ever written into this daemon. Real
# certificates are the front's job; help/tls explains Caddy tls internal /
# certbot / acme.sh instead. Boot never dies on TLS: a missing or unloadable
# cert in local/external mode logs LOUDLY and the daemon starts plain HTTP -
# recoverable-plain beats unreachable-bricked, and Settings still shows why.
# Cookie scar fix rides with this slice: Secure belongs to what the BROWSER
# sees (TLS socket or X-Forwarded-Proto: https), not to a constant - a
# constant-Secure cookie over plain http LAN is silently dropped by every
# browser and login dies without a single console error.
TLS_DIR = STATE / "tls"
TLS_MODES = ("proxy", "local", "external")
TLS_SERVED = False            # True only when main() wrapped the live socket
_TLS_RUNTIME = "proxy"        # mode actually in effect since boot
_TLS_SERVED_CERT = None       # cert path the live socket serves (None = plain)
def _tls_mode():
    # env MARA_TLS_MODE is the operator lock (wins over the UI; keeps systemd
    # authority honest on managed boxes); the owner settings row is the UI path.
    m = (os.environ.get("MARA_TLS_MODE", "").strip().lower()
         or str(get_setting("tls_mode", "proxy", DAEMON_OWNER) or "").strip().lower())
    return m if m in TLS_MODES else "proxy"
def _tls_ext_paths():
    return (str(get_setting("tls_cert_path", "", DAEMON_OWNER) or "").strip(),
            str(get_setting("tls_key_path", "", DAEMON_OWNER) or "").strip())
def _tls_active_pair():
    """(cert, key) the configured mode WOULD serve with, or (None, None)."""
    m = _tls_mode()
    if m == "local":
        c, k = str(TLS_DIR / "host.crt"), str(TLS_DIR / "host.key")
        return (c, k) if os.path.exists(c) and os.path.exists(k) else (None, None)
    if m == "external":
        c, k = _tls_ext_paths()
        if c and k and os.path.exists(c) and os.path.exists(k):
            return (c, k)
    return (None, None)
def _tls_ctx_from(cert, key):
    import ssl as _r4_ssl
    ctx = _r4_ssl.SSLContext(_r4_ssl.PROTOCOL_TLS_SERVER)
    try:
        ctx.minimum_version = _r4_ssl.TLSVersion.TLSv1_2
    except Exception:
        pass
    ctx.load_cert_chain(certfile=cert, keyfile=key)  # OpenSSL checks key/cert MATCH here
    return ctx
HELP_BODIES["shares"] = """
<h2>What a share is</h2>
<p>A <b>share</b> is a copy of your agent that you hand to someone else - a link plus a one-time password. The guest opens <code>http://192.0.2.10/share/<id></code> (example address), types the password, and chats with a clone of your agent. It rides <b>your</b> model keys; it has <b>none</b> of your conversations, memories, or files. The password is shown once at creation - rotate it any time, which signs the old guests out instantly.</p>
<h2>What the guest can and cannot do</h2>
<ul>
<li><b>Chat, only.</b> Every share starts as pure chat. Tools, files, settings, tiers, the API - all fenced off by an allowlist, not a denylist.</li>
<li>The clone is told, in code it cannot be talked out of, that the person across from it is a <b>guest</b> and not you.</li>
<li>Each share has its own message rate limit and its own private namespace; two shares never see each other.</li>
</ul>
<h2>Grants: opening the boxes</h2>
<p>On the Shares page each share has grant checkboxes, <b>all off by default</b>: tools, the clone's own memory, and chat history. A granted tool must be one <i>you</i> can use right now - a clone can never be stronger than its creator, and disabling a tool on yourself shrinks every clone instantly.</p>
<h2>The approval inbox</h2>
<p>If a guest genuinely needs a capability, the clone can send you a <b>request</b> - it appears here on the Shares page, never inside your own chat. Approving adds that tool as a standing grant for the share; denying changes nothing. Requests expire on their own after a day.</p>
<h2>The one-minute presence switch</h2>
<p>Tools in shares live only while <b>you are present</b>: keep the Shares page open and a quiet beacon tells the daemon you are home. Close the tab or step away for more than a minute and every share's tools pause - the clones become chat-only until you come back. Nothing else breaks; chat and your data are unaffected.</p>
<h2>Memory boxes and merge-back</h2>
...
<table>
<tr><th>mode</th><th>who serves TLS</th><th>pick it when</th></tr>
<tr><td><b>Proxy</b> (default)</td><td>A reverse proxy in front (Caddy, nginx, Apache...)</td><td>The standard install. The proxy already has a cert or can get one.</td></tr>
<tr><td><b>Local CA</b></td><td>The daemon itself, using a certificate authority it generates</td><td>No domain, no proxy. You install one small CA certificate on each device you use.</td></tr>
<tr><td><b>Existing certificate</b></td><td>The daemon itself, using a cert you (or certbot / acme.sh / Caddy) obtained</td><td>You already have a real cert and just want the daemon to serve it.</td></tr>
</table>
<p>The status card tells you the truth, including the uncomfortable kind: if you saved a mode but have not restarted, it says <i>restart required</i> and shows both the saved and the running mode. Saving a certificate pair runs a real validation first - a mismatched or unreadable pair is rejected and nothing at all is written.</p>
<h2>Walkthrough A - Caddy in front (recommended)</h2>
<p>Install Caddy per its own docs (<a href="https://caddyserver.com/docs">caddyserver.com/docs</a>), then this is the entire Caddyfile for a real domain:</p>
<pre><code>cairn.example.com {
    reverse_proxy 127.0.0.1:8470
}</code></pre>
<p>Caddy requests, renews, and installs the certificate itself. Leave the daemon in <b>Proxy</b> mode; it stays on loopback and Caddy talks to it there. No real domain? Caddy can be its own certificate authority:</p>
<pre><code>192.0.2.10 {
    tls internal
    reverse_proxy 127.0.0.1:8470
}</code></pre>
<p>(<code>192.0.2.10</code> is a documentation placeholder - use your box's actual address. Then run <code>caddy trust</code> on each device, or export Caddy's root certificate per its docs, so browsers stop warning.)</p>
...
<h2>Walkthrough B - the daemon's own local CA</h2>
<ol>
<li>Settings &rsaquo; TLS &rsaquo; choose <b>Local CA</b> &rarr; <b>Generate</b>. This creates a 10-year CA and a ~2-year host certificate with SubjectAltNames for localhost, this hostname, hostname.local, and 127.0.0.1 - plus any extra names or addresses you type in (bare hostnames and IPs, comma-separated).</li>
<li>Download <code>ca.crt</code> from the card (every signed-in account can fetch it; it is a public certificate, the private half never leaves the box - the TLS directory is mode 0700 and keys are 0600).</li>
<li>Install <code>ca.crt</code> on each device:</li>
</ol>
<pre><code># Windows (PowerShell, as Administrator)
...
sudo cp ca.crt /usr/local/share/ca-certificates/cairn-ca.crt
sudo update-ca-certificates</code></pre>
<p><b>Android:</b> Settings &rarr; Security &rarr; More security settings &rarr; Encrypt or install credentials &rarr; CA certificate. <b>iOS/iPadOS:</b> open the downloaded ca.crt &rarr; Install Profile &rarr; then Settings &rsaquo; General &rsaquo; About &rsaquo; Certificate Trust Settings &rarr; enable full trust for it. That last toggle is easy to miss and the warnings are inexplicable without it.</p>
<p>Re-issuing the host certificate (new name, new address) reuses the same CA, so devices that already trust it keep trusting - nothing to reinstall. The CA itself is never silently rotated: regenerate replaces it deliberately or not at all.</p>
<h2>Walkthrough C - a real certificate from certbot</h2>
<p>Needs a real domain pointing at your network. Install per <a href="https://certbot.eff.org">certbot.eff.org</a>, then:</p>
<pre><code>sudo certbot certonly --standalone -d cairn.example.com</code></pre>
<p>Files land in <code>/etc/letsencrypt/live/cairn.example.com/</code>. In Settings &rsaquo; TLS choose <b>Existing certificate</b>, paste the <code>fullchain.pem</code> and <code>privkey.pem</code> paths, save (validated before it is accepted), restart. Renewals: certbot replaces the files in place, so the daemon just needs a restart after each renewal - or leave the cert with a proxy and stay in Proxy mode, which sidesteps this entirely.</p>
<h2>Walkthrough D - a real certificate from acme.sh</h2>
<pre><code>curl https://get.acme.sh | sh -s email=you@example.com
~/.acme.sh/acme.sh --issue -d cairn.example.com --standalone
~/.acme.sh/acme.sh --install-cert -d cairn.example.com --fullchain-file /opt/cairn-tls/fullchain.pem --key-file /opt/cairn-tls/privkey.pem</code></pre>
<p>Point the daemon at those two installed paths (it must be able to read the key as the user the daemon runs under - <code>--key-file-mode 640</code> with group ownership is the polite shape). Docs: <a href="https://github.com/acmesh-official/acme.sh">acmesh-official/acme.sh</a>.</p>
<div class="card"><p style="margin:0"><b>Please read:</b> commands provided to assist; these tools are very safe, but this project does not control that repository - review before you run.</p></div>
...
<p>A VPN makes your phone and laptop act like they are plugged into the box's network, with zero open router ports. Two well-worn choices:</p>
<ul>
<li><b>Tailscale</b> - WireGuard underneath with account-based key exchange; the "it just works" shape. Install the app on the box and each device, done. Docs: tailscale.com.</li>
...
<p><b>Port forwarding is not recommended without a thorough understanding of network security.</b> Concretely, you take on:</p>
<ul>
...
<p>If you truly must: forward one port, to one machine, TLS-only, with the daemon itself never exposed (it binds loopback anyway - forwarding 8470 accomplishes nothing; you would forward your proxy's 443).</p>
<h2>Firewall basics (the commands you actually want)</h2>
<pre><code>sudo ufw status verbose                    # what is open right now?
sudo ufw default deny incoming             # the sane floor
sudo ufw allow OpenSSH                     # so you do not lock yourself out
sudo ufw allow 443/tcp                     # only if this box serves your proxy
sudo ufw allow from 192.0.2.0/24 to any port 8470</code></pre>
<p>(<code>192.0.2.0/24</code> is a documentation placeholder for your LAN range - and that last rule is <i>example only</i>: with the daemon on loopback there is nothing to reach on 8470 from outside. The 443 rule matters; the 8470 rule documents the boundary.) Check your router too - the firewall on the box and the firewall at the edge are two different doors and both count. Order of preference, restated: <b>VPN > TLS proxy > port forward you understand completely > nothing</b>.</p>
"""
_HELP_TITLE.update({"tls": "TLS & Certificates", "remote": "Remote access & Firewall"})
def _r4_openssl(args, timeout=60):
    return subprocess.run(["openssl"] + list(args), capture_output=True, text=True, timeout=timeout)
def _tls_validate_pair(cert, key):
    """None = pair parses, matches, and is not expired. Error string otherwise."""
    if not cert or not key:
        return "both a certificate path and a private key path are required"
    if not os.path.exists(cert):
        return "certificate file not found"
    if not os.path.exists(key):
        return "key file not found"
    try:
        _tls_ctx_from(cert, key)
    except Exception as e:
        return "OpenSSL rejected the pair (mismatch, wrong format, or unreadable): " + type(e).__name__
    try:
        import ssl as _r4_ssl2
        import datetime as _r4_dt
        import email.utils as _r4_eu
        na = _r4_ssl2._ssl._test_decode_cert(cert).get("notAfter")
        if na:
            _exp = _r4_eu.parsedate_to_datetime(na)
            if _exp < _r4_dt.datetime.now(_r4_dt.timezone.utc):
                return "certificate expired on " + na
    except Exception:
        pass  # expiry peek is a nicety; load_cert_chain above is the real gate
    return None
def _tls_generate(san_extra=""):
    """Local CA (4096 RSA, 10y) + host cert (2048, ~2y) via openssl. Refuses to
    silently rotate an existing CA: reissue reuses it. Raises RuntimeError with
    an operator-readable message."""
    import socket as _r4_sock
    if _r4_openssl(["version"]).returncode != 0:
        raise RuntimeError("openssl is not available on this box - install it, or use mode=external")
    TLS_DIR.mkdir(parents=True, exist_ok=True)
    try:
        os.chmod(str(TLS_DIR), 0o700)
    except Exception:
        pass
    ca_key, ca_crt = str(TLS_DIR / "ca.key"), str(TLS_DIR / "ca.crt")
    host_key, host_csr, host_crt = str(TLS_DIR / "host.key"), str(TLS_DIR / "host.csr"), str(TLS_DIR / "host.crt")
    def _run(args, what):
        try:
            r = _r4_openssl(args)
        except Exception as e:
            raise RuntimeError("openssl " + what + " failed: " + type(e).__name__)
        if r.returncode != 0:
            raise RuntimeError("openssl " + what + " failed: " + ((r.stderr or "").strip()[:300] or "exit " + str(r.returncode)))
    ca_created = False
    if not (os.path.exists(ca_key) and os.path.exists(ca_crt)):
        _run(["genpkey", "-algorithm", "RSA", "-pkeyopt", "rsa_keygen_bits:4096", "-out", ca_key], "CA keygen")
        os.chmod(ca_key, 0o600)
        _run(["req", "-x509", "-new", "-sha256", "-days", "3650", "-key", ca_key,
              "-subj", "/CN=CAIRN Local CA",
              "-addext", "basicConstraints=critical,CA:TRUE",
              "-addext", "keyUsage=critical,keyCertSign,cRLSign", "-out", ca_crt], "CA self-sign")
        ca_created = True
    _run(["genpkey", "-algorithm", "RSA", "-pkeyopt", "rsa_keygen_bits:2048", "-out", host_key], "host keygen")
    os.chmod(host_key, 0o600)
    hn = (_r4_sock.gethostname() or "cairn").split(".")[0]
    sans = ["DNS:localhost", "DNS:" + hn, "DNS:" + hn + ".local", "IP:127.0.0.1"]
    import ipaddress as _r4_ip
    import re as _r4_re
    # 0.6t Y (audit round 7): SAN entries land verbatim in an openssl ext file;
    # strip() only trims ENDS, so a CR/LF/TAB in the MIDDLE of an entry could
    # smuggle extra openssl config lines (e.g. basicConstraints=CA:TRUE).
    # Control chars refused outright; DNS entries must match strict hostname.
    _Y_HOSTNAME_RE = _r4_re.compile(r"^(?=.{1,253}$)[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?(?:\.[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*$")
    for part in (san_extra or "").split(","):
        part = part.strip()
        if not part:
            continue
        if "://" in part or "/" in part or " " in part or any(c in part for c in "\r\n\t"):
            raise RuntimeError("SAN entry looks wrong: " + part + " (want bare hostnames and/or IPs, comma-separated)")
        try:
            _r4_ip.ip_address(part)
            sans.append("IP:" + part)
            continue
        except ValueError:
            pass
        if not _Y_HOSTNAME_RE.match(part):
            raise RuntimeError("SAN entry looks wrong: " + part + " (want bare hostnames and/or IPs, comma-separated)")
        sans.append("DNS:" + part)
    _run(["req", "-new", "-key", host_key, "-subj", "/CN=" + hn, "-out", host_csr], "host CSR")
    ext = str(TLS_DIR / "host.ext")
    with open(ext, "w", encoding="utf-8") as f:
        f.write("basicConstraints=CA:FALSE\nkeyUsage=digitalSignature,keyEncipherment\n"
                "extendedKeyUsage=serverAuth\nsubjectAltName=" + ",".join(sans) + "\n")
    _run(["x509", "-req", "-in", host_csr, "-CA", ca_crt, "-CAkey", ca_key,
          "-days", "825", "-sha256", "-set_serial", str(int(time.time() * 1000)),
          "-extfile", ext, "-out", host_crt], "host cert signing")
    return {"ca_created": ca_created, "sans": sans, "cert": host_crt, "ca": ca_crt}
def _tls_status():
    m = _tls_mode()
    apair = _tls_active_pair()
    shown = None if m == "proxy" else (apair[0] or _tls_ext_paths()[0] or (str(TLS_DIR / "host.crt") if _tls_mode() == "local" else None))
    if m == "proxy" and os.path.exists(str(TLS_DIR / "host.crt")):
        shown = str(TLS_DIR / "host.crt")   # preview the local cert validity before switching
    cert_valid, cert_error = (False, None)
    if shown and os.path.exists(shown):
        kk = None
        if apair and apair[0] == shown:
            kk = apair[1]
        elif os.path.exists(str(TLS_DIR / "host.key")) and shown == str(TLS_DIR / "host.crt"):
            kk = str(TLS_DIR / "host.key")
        if kk:
            cert_error = _tls_validate_pair(shown, kk)
            cert_valid = cert_error is None
        else:
            cert_error = "key path not saved (write-only fields)"
    restart_required = (_TLS_RUNTIME != m) or (TLS_SERVED != (apair[0] is not None)) or (apair[0] is not None and apair[0] != _TLS_SERVED_CERT)
    env_lock = os.environ.get("MARA_TLS_MODE", "").strip() or None
    cpath, kpath = _tls_ext_paths()
    return {"mode": m, "runtime_mode": _TLS_RUNTIME, "tls_served": bool(TLS_SERVED),
            "ca_exists": os.path.exists(str(TLS_DIR / "ca.crt")), "cert": shown,
            "cert_valid": cert_valid, "cert_error": cert_error,
            "cert_path": cpath, "key_path_present": bool(kpath),
            "restart_required": bool(restart_required), "env_lock": env_lock}
def _host_is_loopback(h):
    # H08: the maintenance-door test. "localhost", empty, 127/8 and ::1 pass;
    # 0.0.0.0 explicitly does NOT (bind-all is reachable from the wire).
    h = (h or "").strip().strip("[]")
    if h in ("", "localhost"):
        return True
    try:
        import ipaddress as _h8ip
...
def _tls_wire(server):
    ...
    _cert, _key = _tls_active_pair()
    _TLS_RUNTIME = _tls_mode()
    if _TLS_RUNTIME == "proxy":
        return False
    if _cert:
        try:
...
def main():
    ...
    server = _BoundedHTTPServer((HOST, PORT), MaraHandler)
    # H08 (audit 2026-09-24, K80 "no half-backed fixes") supersedes the F29 R4
    # "never fatal" stance: recoverable-plain was only ever recoverable by
    # someone reaching the box the way they MEANT to - over TLS. A non-loopback
    # bind with a broken or missing cert now refuses to serve at all instead of
